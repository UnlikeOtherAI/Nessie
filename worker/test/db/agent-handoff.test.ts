...
import { buildContext, cleanup, designerHome, seed } from './agent-handoff-fixture.js'
...
runDatabaseTest('a busy Designer DM pends the brief instead of double-running the agent', async (t) => {
  ...
  assert.equal(
    ...
    1,
    'the brief is recorded as a pending row instead (it drains alone: agent-handoff-drain.test.ts)',
  )
