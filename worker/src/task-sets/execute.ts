...
import { changeTaskSetHealth, changeTaskSetHealthInTransaction, TaskSetBlocked, TaskSetWait } from './state.js'
...
const PendingOutcomeSchema = z.union([
  z.object({ result: z.string(), disclosure: TaskSetDisclosureSchema }).strict(),
  z.object({ reason: z.string(), waiting: z.boolean(), offline: z.boolean().optional() }).strict(),
])
...
export const settleTaskSetItem = async (
  ...
  if (set.currentItemId !== item.id || item.currentAttemptId !== claim.attempt.id) {
    throw new RunFencedError(claim.attempt.runId)
  }
  ...
  if (run.inferenceResourceAdmissionId) {
    ...
    if (!released) {
      ...
      await tx.taskSetAttempt.update({ where: { id: claim.attempt.id }, data: {
        status: 'settling', pendingOutcome: taskSetJson(PendingOutcomeSchema.parse(outcome)),
        reason: 'waiting_for_stop_confirmation', statusChangedAt: new Date(),
      } })
      ...
      const admission = await tx.inferenceResourceAdmission.findUnique({
        where: { id: run.inferenceResourceAdmissionId }, select: { state: true },
      })
      await changeTaskSetHealthInTransaction(tx, set.id, {
        reason: admission?.state === 'uncertain' ? 'termination_uncertain' : 'waiting_for_stop_confirmation',
        waiting: admission?.state !== 'uncertain',
      })
      return false
    }
  }
  ...
  if (!stopped && success) await enqueueTaskSet(tx, set.id, updated.revision)
  if (!stopped && !success) await changeTaskSetHealthInTransaction(tx, set.id, {
    ...outcome, waiting: retry, delayMs,
  })
  return true
})
...
export const executeTaskSet = async (deps: Deps, id: string, signal?: AbortSignal): Promise<void> => {
  ...
  try {
    ...
    await withRunExecutorFence(claim.attempt.runId, async () => {
      ...
      try {
        let outcome: Outcome
        if (admitted.attempt.status === 'settling') {
          outcome = PendingOutcomeSchema.parse(admitted.attempt.pendingOutcome)
        } else {
          try {
            ...
            outcome = await processTaskSetItem({
              ...deps,
              searchTools: deps.searchTools ?? ((entry, context) => buildTaskSetSearchTools(deps, entry, context)),
        ...
        await settleTaskSetItem(deps, admitted, execution.token, outcome)
