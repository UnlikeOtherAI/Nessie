...
import { prepareToolAuthorizationInput } from './tool-authorization-input.js'
import type {
...
export const authorizeToolExecution = async (
  ...
  const preparedInput = await prepareToolAuthorizationInput(
    prisma,
    context,
    toolName,
    args,
    toolActorContext,
    emitAudit,
    auth.revalidateApprovalBoundary,
  )
  if (preparedInput.decision === 'deny') return preparedInput
  const canonicalArgs = preparedInput.args
