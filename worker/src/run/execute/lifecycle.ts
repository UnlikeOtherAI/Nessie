...
import { applyReplyBookkeeping, QueueRetryAfterError, type PgRealtimeTransport } from '@nessie/runtime'
...
import { admitRunAgentCore } from './agent-core-admission.js'
/**
...
export const loadRunContext = async (
  ...
  const { consumedSources, coreDocuments } = await admitRunAgentCore({
    organizationId,
    payload,
    prisma,
    run,
