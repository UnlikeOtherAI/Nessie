...
// trigger fire) and the drain side inside the same transaction as the
// run/pending writes: a claim either sees the run still active (and pends, so
// the pending row precedes any drain check) or sees it terminal (and claims
// the slot itself). DeepWater/product-handoff runs keep their own stricter
// invariants and never touch this module.
...
// run. The whole batch stays visible to the follow-up through normal thread
// conversation loading; the latest pending message drives the run's prompt,
// triggerMessageId (restart replay), interactivity (budget exemption), and
// actor context. Returns the follow-up run id, or null when there is nothing
// to drain (no pendings, or a run is already in flight again).
