...
- DeepWater is usable as a tool by any permitted agent, but **default OFF with an explicit per-agent grant required**: enabling DeepWater for a team (owner-only team-enablement toggle) provisions a **team-scoped** tool-projecting `McpServerInstance` from the `deep-water` catalog entry, resolves the endpoint from `DEEP_WATER_MCP_URL` (enable fails loudly with `DEEP_WATER_MCP_URL_UNSET` when unset — never a dead instance), installs an HTTP transport, and projects the manifest's `research_*` tools into `ToolRegistryEntry` as `active` (surfaced as `mcp_research_*`); disabling removes it (teardown keyed on the instance's own catalog-entry name). The projected DeepWater tools and the `deep_water_run_update` builtin are flagged `requiresExplicitGrant`, so team scope alone never exposes them — an agent (PA or shared) sees them ONLY when its `toolPolicy` sets an explicit allow (`=== true`); an absent/inherited verdict denies (opposite of ordinary scope-exposed connectors, which are unchanged). First-party team-enable stands in for the manual install + admin-approve gate, per-user OAuth resolves at dispatch, and `deep_water_run_update` (not PA-only) writes the durable run record back with tenancy taken strictly from the run context (same team + same thread; `knowledgePageId` validated against the org). A re-enable never clobbers a manually-probed install's schemas.
