...
- **Device builds are always installable release builds.** When the user asks for a build, build and install it directly on their named iPhone, iPad, or Android device as a durable, normally usable app — never substitute a debug, development-client, simulator, or emulator build unless they explicitly request one. Use the applicable signing, provisioning, distribution, and device-install workflow so the resulting app continues to launch outside a developer session; if the required device, signing identity, profile, or distribution access is unavailable, state the exact blocker rather than delivering a debug build. A screen headed **“Development Build”**, **“No development servers found”**, or **“Enter URL manually”** is a failed deployment, not an acceptable handoff: replace it with the standalone app and visually verify the Nessie login or workspace screen on every named physical device before reporting the build complete.
