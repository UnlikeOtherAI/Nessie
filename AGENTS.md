...
- **Postgres-backed suites share one database and run concurrently.** CI's `pnpm -r --if-present test` (and `turbo run test`) runs the api and worker packages at the same time against a single `DATABASE_URL`, and `node --test` runs files within a package in parallel too. So a DB-backed test is never alone:
