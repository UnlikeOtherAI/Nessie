...
Still planned in this step: `Sheet` for the eight drawers, `presentation:
'panel' | 'full'` for Flows, and the adoption of the remaining bespoke overlays.
Pinned by `admin/test/navigation-overlay.test.ts`,
`admin/test/dialog-shell.test.ts`, `admin/test/place-popover.test.ts`,
`admin/test/popover.test.ts` and `admin/test/card-viewport.test.ts`.
