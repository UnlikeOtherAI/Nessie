...
**One retry owner per occurrence:** A dispatch that has persisted a delivery hands that occurrence to the delivery ledger. The scheduler settles the recurring cadence from the claimed occurrence's original `next_run_at` and never turns a retry instant into a new scheduled key; this holds for one-offs, cron schedules and intervals. A final recurring occurrence can pause future cadence while its already-persisted delivery finishes retrying; a classified `error` state is never revived by settlement. A person pausing or cancelling a trigger disables it, which cancels every pending delivery retry. If dispatch fails before a delivery can be recorded, no delivery owns it, so the scheduler retains its fenced claim until the ordinary lease expires and retries the same original occurrence. This keeps both retry paths crash-safe without letting a successful delivery retry create a second kickoff.
