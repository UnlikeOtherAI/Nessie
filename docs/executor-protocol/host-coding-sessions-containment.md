...
`policy_suspended`, `policy_ended`, `work_limit` and `machine_reassigned`, all
pinned by a CHECK),
...
- one ticket's work ending its sessions, each named by id, for the ticket's
  six reasons: the ticket entering an end column (`ticket_left_flow`), the
  ...
  work hitting a limit (`work_limit`), or the machine staying offline past
  the trigger's `waitingMachineHours`, so the work was queued again for
  another machine and the sessions it left here close when this one
  reconnects (`machine_reassigned`, T5; migration
  ...
  A request still expires after a day like every other, counted from when it
  was written. A ticket's sessions have an owner key of their own ("Owners" in
