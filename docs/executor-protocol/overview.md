...
denied after normalizing dot segments and both slash spellings, so aliases such
as `./.nessie-executor-promotions` and Windows backslash paths cannot expose it;
it is excluded from sandbox snapshots and listings, and all sandbox/native manifest
