...
- `draft` — editable by creator, not runnable outside the authoring environment
- `testing` — build/test run in private sandbox
- `private_sandbox` — creator can run it in isolated sandbox only
- `shared_unreviewed` — optionally visible to selected users/scopes, still sandbox-only
- `pending_review` — frozen for DevOps/security/platform review
- `changes_requested` — reviewer feedback sent back to creator in Nessie thread/DM
- `approved` — version approved for publication
- `published` — available in library for assigned scopes
- `revoked` — disabled for new runs, retained for audit

Approval is per version, not per plugin name. Any code change creates a new version that must be reviewed again unless explicitly fast-pathed for metadata-only edits.

### Review and Promotion

Review is split into two decisions:

- `distribution approval` — who may discover/assign the plugin
- `runtime approval` — where and how the plugin may execute
