...
- provider and model identity must be stored as raw strings, not fixed enums. Agent Designer options come only from Nessie's `GET /api/agents/models`, which calls Ledger's token-scoped `GET /v1/models` without exposing the deployment credential and with the same signed Nessie/UOA provenance as inference; it admits direct service rows advertising OpenAI-compatible `chat/completions` only, validates create/update pairs against the current catalog, and routes uncompiled services through the generic OpenAI-compatible Ledger transport. The route returns the catalog already ordered for display — provider name ascending, then newest first within each provider, comparing labels as alternating text and version chunks (`api/src/services/agent-model-order.ts`) so `GPT-10` leads `GPT-5` and `GPT-5` leads `GPT-4.1`. Agent Designer renders it in a type-to-filter combobox (`admin/src/components/features/agents/designer/ModelCombobox.tsx`): every whitespace-separated term must match the display name, model id, provider or description, and each row carries an 8px subtitle with the model id and the catalog description. Creating an agent preselects the leading catalogue entry, and the save action states its blocker in-page ("Add a name", "Pick a model") rather than leaving a disabled button unexplained. The Design Assistant sees that same catalogue in its system prompt, in the same order, and sets the field with one `set_model` tool taking `{ model, provider }` together — the form resolves a selection by matching both, so a half-applied pair would read as no model at all; the admin applies the call only after resolving the pair back to a catalogue entry, leaving the existing selection alone when the assistant names something the catalogue does not carry. The assistant chooses a model itself rather than asking, and the greeting names exactly the fields it has tools for (name, role, model, system prompt, tools).
