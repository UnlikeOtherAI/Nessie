...
- **Scoped sharing**: scope rules per [docs/standards/mcp-connectors.md](mcp-connectors.md); shared-scope installs keep the `pending_review` tool gate (user-scope auto-activate). See [docs/external-tool-integration.md](../external-tool-integration.md) §2.
