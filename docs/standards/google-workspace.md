...
- **Calendar and Meet have a first-connection doorway.** Your settings → Connected
  accounts → Mail and calendar → **Connect Calendar or Meet** opens a capability
  picker that sends only selected Calendar and Meet capability ids to the
  existing Google OAuth start route. The same tab's **Connect email** → Google
  keeps its Gmail-read and Meet default. Either way the account is one row in
  that tab's table, and its page's Permissions rows and in-chat grant cards
