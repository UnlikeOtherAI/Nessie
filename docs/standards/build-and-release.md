...
- **The Nessie Executor menu bar DMG is the one macOS artifact that must be installable with no bypass at all.** `executor/packaging/macos/build-dmg.mjs` signs it with the `Developer ID Application` identity in `NESSIE_EXECUTOR_SIGNING_IDENTITY` for the team in `NESSIE_DESKTOP_SIGNING_TEAM_ID`, notarizes app and image with `notarytool` (an App Store Connect key — `APP_STORE_CONNECT_API_KEY_ID`, `APP_STORE_CONNECT_API_ISSUER_ID`, `APP_STORE_CONNECT_API_KEY_PATH` — or `APPLE_ID` with `APPLE_APP_SPECIFIC_PASSWORD`), staples both, and proves the result with `spctl`. A host with no certificate may build only the `UNSIGNED-DEVELOPMENT-DO-NOT-INSTALL` image, which the release workflow can neither produce nor publish; there is no degrading in between. Do not copy the neighbouring ad-hoc desktop DMG job's stance into it. Recipes, every environment variable, and the refusals: [docs/running-the-apps/executor-menu-bar-macos.md](../running-the-apps/executor-menu-bar-macos.md).
- **Nessie Desktop's Developer ID build nests the executor menu bar app** at `Contents/Library/LoginItems/Nessie Executor.app`, so a Mac with Desktop installs nothing extra to become an executor. `bundle.macOS.files` resolves its keys against the bundle's `Contents` directory, and the entry lives in `desktop/src-tauri/tauri.executor-menubar.conf.json`, which only `tauri:build:executor` passes. **Signing is inside-out:** `desktop/scripts/prepare-executor-menubar.mjs` seals the nested app with its own entitlements *before* Tauri seals the enclosing bundle, and the whole thing is notarized once as Desktop. Never `codesign --deep` the outer app to cover it — that applies Desktop's entitlements to the helper. The sandboxed Mac App Store build keeps `bundle.resources: []` and is never handed the nesting configuration, so it cannot gain the executor through a config merge; `desktop/src-tauri/src/lib.rs` asserts that split. The published direct-download macOS DMG is ad-hoc signed and does **not** nest it, because an ad-hoc build offers no executor controls at all — that Mac uses the standalone `Nessie-Executor-macOS-*.dmg`.
