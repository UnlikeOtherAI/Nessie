...
any new dispatch branch. Produce a reviewed authority/precedence test matrix
and the complete new schema; feature remains off.
...
Resolve Sol's verified findings before implementation is called complete;
...
| Run security/lifecycle | `worker/src/run/execute/disclosure-basis.ts`, `crash-checkpoint.ts`, `cancel-stop.ts`; `api/src/services/runs.ts`, `run-continuation.ts` | Full prompt-recipient check before every dispatch; cancellation/lease fencing; resume exact pin; retry doorway for offline terminal failure. |
...
| Usage/health | Existing `recordInferenceUsage`, `applyBudgetGate`, `UserAlert` pipeline and worker control sweeps | Structural local billing source, currency exclusion, token metering, per-transition alerts, bounded attempt/TTL sweeps under existing locks. |
...
| Model home | `admin/src/components/features/agents/designer/ModelCombobox.tsx`, `AgentDesignerForm.tsx`, `ModelUnavailableNotice.tsx` | Compose the same local setup/status feature, consumed `section=model` deep link and explicit unavailable selection. |
...
1. Expand schema with nullable local fields and no fabricated host/presence
...
4. Install signed host-capable client builds, then enable the deployment
...
5. Rollback is disable admission → fence/drain attempts → leave local agents
...
| Live authority | UOA team/org removal, demotion, epoch rotation and outage; direct custodian and scheduled stored-identity paths; no new user/profile/member rows; unbound local tenant behaviour; private agent unreadable to org admin. |
| Database races | Cross-org FKs, one active binding, prepare/confirm edit race, concurrent claim, replayed challenge, stale epoch heartbeat/goodbye/result, duplicate sequence mismatch, exactly-once terminal usage, delete/revoke during lease. |
| Loopback transport | IPv4/IPv6, alternate local port, redirect rejection, public/LAN/metadata/rebinding refusal, malformed JSON, oversized response, proxy-env bypass attempt, unavailable runtime, successful empty inventory distinct from failed scan. |
| Discovery | Already running Ollama on macOS/Windows/Linux; installed but stopped; service versus user account; WSL inaccessible from host; aliases; remote/cloud entries; tag change, model removal, unsupported version; no automatic exec/pull/create/delete. |
...
| Disclosure | Public context; own private agent; shared agent on another person's host denied private-author content; unknown lineage; tool adds restricted source mid-run; resumed checkpoint; no prompt bytes persisted/delivered before the host-recipient gate. |
...
| Native distribution | Signed macOS direct build, Windows MSI/NSIS, Linux package; no workspace/VM for direct mode; secure-store unavailable state; no secret in argv/webview/log; app exit terminates bridge; executor remains independent. |
...
## Open decisions and recommended resolution

These are engineering qualifications to resolve during the first implementation
slice, not permission questions or reasons to ship a weaker implicit fallback.
...
| Ollama local/cloud discrimination | Capture 0.34.1 local and remote metadata and add strict fixtures before enabling selection. Unknown locality is ineligible. |
...
1. An authorised person attaches an **existing** agent to an installed local
...
6. Pairing, per-message proof, tenant/epoch/fence checks, prompt-recipient
   disclosure, cancellation, expiry, encrypted payload retention and loopback
   restrictions pass adversarial and two-instance tests.
...
8. Durable headless browser flows, native platform checks, actual Windows
