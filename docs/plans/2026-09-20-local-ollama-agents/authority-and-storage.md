...
| `AgentLocalInferenceBinding` | `id`, `organizationId`, `agentId`, `hostId`, model name, manifest digest, observed capability snapshot/hash, effective host `numCtx`, agent edit revision, prepared editor's product credential generation and (where applicable) UOA subject/token generation, prepared host authorization revision and connection epoch, policy version, `revision`, `consentDigest`, consent author/reference and time, status `pending/consented_pending_activation/active/needs_rebinding/revoked`, typed reason. Partial unique active binding per agent; historical rows retained. Composite FKs prove organisation equality; owner subject must equal custodian subject. |
