...
| F-ROW-02 | click `Plan` | the row has `aria-selected="true"`, computed `background-color` equals `--accent` and `color` equals `--on-accent`; the document opens full-surface with a Back button; Back returns to the same columns (same folder, scroll and selection: column 1's `Contracts` row still `aria-selected` with `--overlay` background) | `selected-pill.png` |
...
| F-AGENT-01 | (if an agent with a home exists) open its Documents tab | column 0 is the home's listing under the existing warning notice, titled with the agent's name (no ` — Documents` suffix); the column header's **Open** button is absent (it is the scoping agent). Browsing the same folder from `/knowledge-base` shows **Open** in the column header and navigates to `/agents/<id>` | `agent-docs-tab.png` |
