...
- Evidence: `packages/config/src/index.ts:8-13` and `packages/schemas/src/identity.ts:12-18` declare `saml`; `api/src/services/external-auth.ts:34-42` rejects every non-OIDC type; `docs/deployment-modes-and-auth-spec/overview.md:115` says "SAML via gateway/adapter"; absent: searched `api/src`, `worker/src`, `packages/*/src` for `saml`/ACS/SP-metadata/assertion handling, found only the enum declarations.
...
- Update (2026-07-23): **Threshold + block alerting shipped.** The budget gate now pushes a notification to org owners + scope managers the first time a scope crosses `warnThresholdPercent` and the first time it blocks a run, once per period, through the existing push pipeline and respecting push preferences (`worker/src/run/execute/budget-alert.ts`, `worker/src/control/budget-alert-dispatch.ts`; dedupe via the `budget_alerts` marker). Failed-run spend is now persisted and readable by outcome at `GET /api/ledger/tokens/by-outcome`. **Still open:** burn-rate/anomaly detection (the "hourly spend > Nx baseline" alarm) and a scheduled evaluator independent of run activity.
