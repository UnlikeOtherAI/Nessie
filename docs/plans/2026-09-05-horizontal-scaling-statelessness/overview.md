...
| 4.13 | — | Give the worker pool a termination grace longer than the worker's whole shutdown budget, and set `NESSIE_WORKER_DRAIN_TIMEOUT_MS` explicitly. The worker now drains for 25 s, settles abandoned handlers for 5 s and bounds teardown at 10 s, so 40 s must fit inside the platform's grace; the terraform sets no grace on the pool and the shared `NESSIE_SHUTDOWN_TIMEOUT_MS` it does set is the API's budget, which the worker never reads. | S |
