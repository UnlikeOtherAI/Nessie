...
| 5.14 | 2.5 | Give `run_document_sessions` its own claim and make every session write ride it. `runs.executor_token` fences the RUN; the four terminalisers write the session table directly and it has no claim column, so an executor fenced out of its run — or merely stalled past 3.3's reaper — can still write the session, and `knowledge-compose.ts`/`knowledge-edit.ts` end with an unconditional `update` by id that would turn a reaped `failed` back into `saved`. The fence is the easy half; the design question is what a superseded executor should do about the page and attachment it has *already* written, since by then the document exists in the knowledge base and only the popup's record of it is in dispute. The queue solved the same problem one layer down and its answer is the model: 5.11 shipped a cross-process fence on `(id, attempt)` for `acknowledge`, `nack` AND `renewLock`, so a superseded worker's write matches nothing. `claimSettle` remains the in-process single-writer gate beside it; the session table needs the cross-process half. | M |
