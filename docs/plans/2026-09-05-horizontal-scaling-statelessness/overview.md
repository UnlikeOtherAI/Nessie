...
| 3.3 | 2.5 | Document-session reaper: `streaming`/`saving` sessions whose run's executor heartbeat has gone silent flip to `failed` with `errorReason: 'executor_lost'`. Built in the **worker's** periodic sweeps under `withSweepLock`, not the API maintenance sweep this row first named — the API's always-on cost is what pins it to a minimum instance, and a fifth API sweep works against ever letting it idle. | S |
| 3.4 | 5.13 | `push.dispatch` enqueues carry an idempotency key and the handler claims a delivery row before sending. | S |
| 3.5 | 2.2 | **Done.** LISTEN reconnect re-reads the backlog for every registered connection from its watermark. The transport fires `onListenRecovered` after a *re*-connect and never after the first (`packages/runtime/src/realtime.ts`); the hub answers it by re-hydrating every registered SSE connection through the same path a fresh one takes, extracted for that reason into `api/src/realtime/connection-hydration.ts`. **Deliberately not covered:** a connection still mid-hydration is skipped — the hook runs after the `LISTEN` was re-issued, so its own in-flight read already sees the log at or after live delivery resumed — and WebSocket connections, which carry no watermark and have no replay to re-read. A per-connection failure is logged, not rethrown. **Was never what the chaos smoke's SSE check was failing on** — that check demanded replay of the five event types the hub deliberately never replays, and no LISTEN drop occurs in its scenario at all. | M |
| 3.6 | — | **Done — decided: `delegate` AND `spawn_subtask` lose `safe: true` and are claimed in `run_tool_effects`.** `agents` was already in `EFFECTFUL_TOOL_CATEGORY_IDS`, so the flag alone excluded it, and the flag was simply wrong: `safe` says a call only reads, and a delegation does not — the sub-agent inherits the parent's builtins minus `delegate`, so it can send mail or file a ticket. The durability defect was real, not bookkeeping: in the window the crash checkpoint cannot cover, a resumed run re-issued the delegation, and the second sub-agent's own effectful calls carried NEW tool-call ids matching no earlier row, so nothing deduped them. The parent's `delegate` call is the one id stable across executions, so claiming it replays the recorded digest instead of creating a second sub-agent. **The alternative was rejected:** restricting the sub-agent to read-only tools would change what the product does — inheriting the parent's toolset is the feature's design — rather than fix durability. **Deliberately still uncovered:** the sub-agent's own calls, dispatched inside `runDelegate` below the ledger's seams under ids belonging to the sub-run, so an interrupted delegation reports one unknown outcome for the delegation as a whole. **`spawn_subtask` was the same defect in the same file with the same one-line fix, and the worse of the two:** a re-issued `delegate` duplicated a transient sub-agent, while a re-issued `spawn_subtask` leaves rows a person sees — `runSpawnSubtaskTool` commits a child `Agent`, `Run` and `Task` in one transaction and enqueues that run, and nothing collapses the repeat (the child's name embeds a fresh `randomUUID().slice(0, 8)`, and the enqueue's idempotency key is built from that new child's id). Shipping one of two identical flags would have left the more visible failure behind. **Noted, not fixed here:** `spawn_subtask` has no per-run fan-out cap of its own — `SUBTASK_CHILD_DENIED_TOOL_IDS` bounds depth, not count — which is a budget question independent of the ledger. | S |
...
| 5.4 | 2.7, 2.9 | **Done.** Three parts, and the first was already half-shipped: the `sse-ref`/`ws-ref` envelopes from `298a15492`/`7e45b5bed` already carried an oversized **durable** publish by row id, so this extended that mechanism rather than adding a second — every door to `pg_notify` now measures the cap in one helper, and the **rowless** lanes (`publishSseEphemeral`, a ws publication whose scopes name no organization, the session revocation) drop and log instead of raising, since they have no row to point at. Resolution of arriving notifications is now serialised on the transport, closing the ordering caveat the ref form's own comment admitted: a ref's re-read costs a round trip, so an oversized event could reach the fan-out behind a smaller one published after it and a forward-only watermark would skip it for good. **2.9:** the replay query asks for `MAX + 1` — counting rows cannot tell a page that ended *on* the cap from one it *cut* — and a truncated page writes an id-less `realtime.gap` frame that `admin/src/facades/realtime/realtime-gap.ts`, mounted once in `AdminShellLayout`, turns into a full React Query invalidation. **Retention:** `thread_stream_events` joins the existing `realtime_events` sweep — same 24 h window, same `realtime_prune_state` cadence row, same `withSweepLock` leader — plus a `created_at` index (`20260909110000_thread_stream_events_retention`), because the sequential scan accepted on the small table is not acceptable on one that grows a row per streamed token. | M |
...
| 5.7 | 6.4 | **Done.** Above `storage.signedDownloadMinBytes` (8 MiB) every attachment download route answers `302` to a signed URL instead of piping the object through the API. 8 MiB is the drain budget, not the request timeout: Cloud Run's SIGTERM-to-SIGKILL grace is a fixed ten seconds (4.9) and at a pessimistic 1 MB/s downlink that is about what completes inside it; under the threshold the proxy keeps the ETag/304 round trip and one origin for the client. The signature grants one GET on one key for 60 s with content type and disposition pinned into it — 60 s because a store validates on arrival and then streams for as long as the bytes take, so the window is the handover and nothing is bought by widening a bearer capability. **Authorisation is unchanged and strictly upstream:** each route runs its existing ACL, then calls `fileService.openDownload`, which re-reads the row by id, refuses a different `organizationId`, and takes the key off the row rather than the request. The 302 is `private, no-store` + `no-referrer`. **Off unless `storage.publicEndpoint` is set,** which is deliberately not derived from `storage.endpoint`: production MinIO is `nessie-minio:9000` on a private Docker network, and setting the new variable asserts two things code cannot check — clients can reach the address, and it allows the admin origin by CORS, because every admin consumer (`downloadAuthedPath`, `useAuthedObjectUrlFromPath`, `useAuthedTextFromPath`) follows the redirect from `fetch()`. `filesystem`, an unset public endpoint and a throwing signer all fall back to the proxy, so nothing here can fail a download. Wired in the Cloud Run terraform (bucket CORS + the env var); Hetzner keeps proxying until MinIO has ingress. | M |
...
| 5.9 | 1.13 | **Done.** `summarizeRateLimitWindows` in `@nessie/db` reads every configured bucket's **current** window out of `rate_limit_buckets` in one statement — the rules arrive as parallel arrays and `unnest` floors each bucket's own `windowMs` from the same `NOW()` the counting statement uses, because buckets run on different window lengths and no single "recent" cutoff is honest for all of them. The bucket list comes from `RATE_LIMIT_BUCKETS` + `rateLimitForRules`, the pairing that is already compile-checked, so a new limiter appears on the page without anyone remembering to add it. **The response labels which numbers are which:** `rateLimit.deploymentWide` (fleet-wide, `source: 'rate_limit_buckets'`, only buckets actually counting, ordered lockouts-first) beside `rateLimit.thisInstance` (this process since its boot). Per-process counters were kept, not deleted, because `storeErrors` counts hits that never reached Postgres and so left no row to read back, and a replica's share of `checks` is a cheap load-balance sanity check — but they are never presented as fleet totals. A failed store read answers `available: false` rather than zeros. Surfaced on the admin's System Health page as two separately-titled blocks. (The three deletions this row also carried — `apiUsageTracker` (audit 1.12), `pubsub-queue.ts` with the worker's fallback warning (audit 5.14), and the Redis config stub with its `hasRedis` capability — have landed. `config.queue.provider` still accepts `'pubsub'`; retiring that enum value goes with the Pub/Sub terraform module in 4.5.) | S |
...
| 5.12 | — | A user-requested terminate of a `docker` execution environment outside `local` can be claimed by a worker that is not the container's host, get `No such container`, and record `terminated` for a container still running. Bounded, because provisioning docker outside `local` is now refused, so only pre-upgrade containers are exposed — but the database should carry the honest state the operator message currently carries only in prose. Delivered on `claude/hs-execution-honesty`: a provider terminate returns an outcome rather than only metadata, and `docker` answers `unverified` when `docker rm -f` says `No such container` anywhere but `local` — the one mode with a single daemon, where that answer is proof. `persistTermination` writes `terminated` only for a verified outcome; an unverified one leaves the instance `failed` with `EXECUTION_TERMINATE_UNVERIFIED`, its `provider_instance_ref` and `terminated_at IS NULL` intact, so the container is findable by its `nessie.instance-id` label on the runner named in the row. | S |
| 5.13 | — | `persistProvisionSuccess` returns `false` without throwing when either of its two conditional writes matches zero rows, so the transaction commits whatever partial state they produced instead of rolling back. A `throw` inside the callback is the one-line fix, but it changes the failure path's control flow and deserves its own change. Delivered on `claude/hs-execution-honesty`: a private `ProvisionPersistConflict` is thrown inside the callback and caught at the transaction boundary, so the rollback happens and the function still returns `false` — the caller keeps running `cleanupProvisionedInstance` for a handled failure, and only a real error still reaches `markProvisionFailure`. | S |
| 5.14 | 2.5 | **Done — decided: nothing a superseded executor already wrote is deleted.** `run_document_sessions.claim_token` stores `runs.executor_token` as it stood when the session was opened, and every session write — the recorder's `terminalize`, the failure path's `finalizeOutstanding`, both saves and both `saving` claims — goes through `settleDocumentSession` (`worker/src/run/execute/document-session-claim.ts`), which asks in one statement for the session to still carry this claim AND the run to still carry the same token. A superseded writer matches nothing and says so at error level with both tokens; the two saves are fenced on the claim ALONE and never on the status, because by then the document exists and a stalled-but-still-holding executor's save must beat a reap that was wrong. **The design question, answered:** the page and attachment stay. The document is real and verified, a fenced-out executor is the worst process in the deployment to be issuing deletes, and `run_tool_effects` already carries the outcome across the takeover so the successor answers from the record rather than writing a second page. Nothing is orphaned — the page is a `.md` document in its space, reachable like any other. **What is left is a product call, not an engineering one:** the popup's row for that session still says the run was lost while the document it describes exists, and the options are (a) leave it, with the refusal in the log, (b) let the successor adopt its predecessor's document window, or (c) give the window a "saved elsewhere" state. Repairing it by letting the superseded executor write the row is the defect, not an option. **The reaper now defers to the claim** rather than inferring from the run: it reaps a session whose claim is not live, with the two executor-is-coming states (parked, drained) named explicitly, so a session stranded by a takeover is collected on the first pass instead of waiting out the whole resumed run. Migration `20260909130000_document_session_claim` is a nullable column no deployed build reads. | M |

One defect adjacent to 5.12/5.13 and delivered with them on
`claude/hs-execution-honesty`: `provisionDocker` used to ADOPT the container
holding a name `docker run` reported as already in use. A `containerName` pinned
on a template is shared by every instance launched from it, so two instance rows
ended up carrying one container id and terminating either destroyed the other's
live environment — the pinned-name collision 5.2's `deriveGcloudProviderInstanceRef`
refuses, except that adoption made it succeed silently. A container is now adopted
only when its `nessie.instance-id` label names this same instance (the crash-retry
case); anything else fails with `DOCKER_CONTAINER_NAME_IN_USE`, and the system
labels are stamped after a template's own so the proof cannot be forged.
