...
| 2.3 | 1.4, 1.7 | Boot work leaves boot: `pnpm --filter @nessie/api reconcile` runs policy seeding, the protected-grant backfill, PA default grants and the credential sweep after `migrate deploy` in `redeploy.sh` and in the Cloud Run Job. Policy seeding gains `pg_advisory_xact_lock` per organisation and a partial unique index on `(organization_id, scope, scope_id, resource_type, action, effect)`. Boot then only connects and listens. | M |
...
| 2.5 | 4.1 | Scoped settings: `@@unique([organizationId, key, scope, teamId, userId])` plus `upsert`; a migration collapses existing duplicates keeping the newest. | S |
...
| # | Fixes | Item | Size |
|---|---|---|---|
| 4.1 | 1.10, 6.1, 6.3, 6.2 | Boot fails outside `local` when storage is `filesystem`, when the `docker` execution provider is enabled, or when a toolset bundle configures `allowedRoots`. Delete the legacy buffer-only `gcs` backend. | S |
