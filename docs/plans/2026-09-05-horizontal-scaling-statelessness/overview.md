...
| 5.9 | 1.13 | Per-process limiter stats on `/api/ops/health` are read from `rate_limit_buckets`. (The three deletions this row also carried — `apiUsageTracker` (audit 1.12), `pubsub-queue.ts` with the worker's fallback warning (audit 5.14), and the Redis config stub with its `hasRedis` capability — have landed. `config.queue.provider` still accepts `'pubsub'`; retiring that enum value goes with the Pub/Sub terraform module in 4.5.) | S |
