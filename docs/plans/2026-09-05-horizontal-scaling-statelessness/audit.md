...
| # | Sev | Finding | Where |
|---|---|---|---|
| 4.1 | DEGRADED (corrected during implementation) | `writeScopedSetting` is find-then-create inside a transaction, so two concurrent saves of one setting race. **This entry originally read BLOCKER and claimed the race produced duplicate rows; that premise was wrong.** Migration `20260903130000_scoped_settings` already carries three partial unique indexes (organisation, team and user scope) plus `scoped_settings_scope_target_chk`, so duplicates were never representable and `resolveFromRows` could not read two rows for one target. The real defect is narrower: the loser of the race fails with `P2002` instead of updating the winner's row. | `packages/runtime/src/scoped-settings.ts:184-233`; `api/prisma/migrations/20260903130000_scoped_settings/migration.sql:33-45` |
