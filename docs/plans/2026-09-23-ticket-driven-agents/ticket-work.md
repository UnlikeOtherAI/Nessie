...
| `wakeCount`, `activeMs`, `costUsd`, `lastWakeAt`, `lastWakeReason` | Limits and the chip. `activeMs` pauses while the record is `queued` or `waiting_machine`, and while an open question waits for a person (see [Quiet wake](#quiet-wake-and-the-sweep-t3)) |
