...
> says otherwise. Landed since: the server half of **Phase 1** (local-password
> stack hard-gated to `local` mode — requirement 0.1), **Phase 2** (principals
> keyed by UOA subject), **Phase 6**'s directory cache, and most of **Phase 5**
> (workspace rosters and invitations now run on the UOA org API in backend
> mode — requirements 3a + 3b). The rows and sequence entries below carry what
> shipped and what is left.
