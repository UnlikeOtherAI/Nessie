...
- **The category owns its own line.** Sharing row 2 with the attribute badges truncated it to `Project Manag…` on a grid tile, and a category the eye cannot read is a filter nobody can use. Category sits on its own line at full card width; the badges drop to the line below it.
