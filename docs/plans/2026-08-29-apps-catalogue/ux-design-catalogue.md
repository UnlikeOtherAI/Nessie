...
│ │ ▣  │  Development                 │  row 2: category, on its own line
│ │    │  🌐 ✓                        │  row 3: attribute badges (hover = hint)
│ └────┘                               │
...
| 5 | **AUTH_EXPIRED** | instance exists, auth invalid/expired | filled dot + count | when >1 | `warning` | `Needs reconnecting` | yes | **Reconnect** (primary) |
| 6 | **ERROR** | probe/connection failure, not auth | filled dot + count | when >1 | `danger` | `Connection error` | yes (detail explains in words) | **Retry** (primary) |
...
- **The category owns its own line, and the identity block is one tight stack.** Sharing row 2 with the attribute badges truncated the category to `Project Manag…` on a grid tile, and a category the eye cannot read is a filter nobody can use. Category sits on its own line at full card width; the badges drop to the line below it. Title, category and badges then hug at a uniform 4px, each following the one above — the title reserves **no** minimum height. It used to reserve two lines so every card's description began at the same y, which left a one-line title floating 20px above its own category and made the block read as three unrelated rows. Cards still end level because the footer is `mt-auto`, so that reservation bought an alignment nothing else needed.
- **Every mark on a card explains itself on hover, focus and tap.** The attribute badges and the status dot are all `components/primitives/HoverHint.tsx` — a real `<button>` that opens a titled two-line panel (`label` in bold, `description` under it). Not a native `title`: that is invisible on touch, unreachable by keyboard, and cannot carry two lines. `HoverHint` was extracted from `AppIconBadge`, which is one of **five** hand-rolled tooltips in the admin (`ReactionPills`, `UserStatusEmoji`, `RailTooltip`, `GroupDmSidebarLabel` are the others); only the two apps-card call sites are on the primitive so far, and moving the rest is a separate migration.
...
- **The words are not lost, they split in two.** `AppCardStatus.label` is the state's name — the hint's title, and what `AppDetailHero` renders as a full `Pill` — and `description` is the sentence under it, saying what the state means or what to do. The count nuance lives in the sentence, never in the label: with several accounts behind an unhealthy state the derived state proves only that **at least one** needs attention, so that is exactly what it says. Labels carry no `●`/`⚠` glyph any more: the card draws the mark, the hint and the hero write the words.
