...
| **Root `pnpm lint` is red on main** | `scripts/lint-markdown-structure.mjs` (step 2 of root `lint`) fails on main: `docs/plans/2026-08-13-live-data-dashboards.md` and `…-live-document-streaming.md` are both 1,088 lines (limit 1,000). The lint-gated build path is currently failing for everyone | Split the two docs per the lint's own prescription (`<name>/overview.md` + chapters) |
