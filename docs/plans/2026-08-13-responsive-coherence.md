...
Also landed 2026-08-14: D6 (sidebar ARIA live-clamp within [min,max]), D7/D8 (measured header leading from observed content + intrinsic-row observation), D10 (drag hygiene: pointercancel/blur, frame coalescing, persist-on-end, thread-separator keyboard path), D14 (stale 900px comment) — Playwright-verified. Also landed 2026-09-03: D9 (safe-area ownership — the native injection now only publishes the inset custom properties on :root, and `styles.css` owns the selectors that consume them at any wrapper depth, aside and main alike; ResizableSidebar and ResizableThreadPanel no longer hide a surface from the clearance) and D13 (closed as gone, not fixed: `McpAppStorePage` was deleted by d36cc40e on 2026-08-31, and every remaining multi-column consumer — WorkflowsPage, IntegrationsPage, TriggersPage, ToolsPage — already wires per-column `onBack` and a depth-tracked `activeColumn` through the ColumnBrowser contract). Open: D11 (popover clipping-ancestor placement), D12 (useMediaQuery is deleted, so moot), Phase 1 edge matrix as durable tests. Phase 5 (per-surface container conversion) is delivered by the navigation framework — `docs/navigation/overview.md` §5, built from `docs/done/2026-09-01-navigation-motion-system.md`: every page's container is decided by `useNavigationLayout()`, never by a breakpoint of its own.
