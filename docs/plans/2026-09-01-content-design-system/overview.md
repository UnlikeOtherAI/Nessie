...
mobile tab bar), page headers (`ScreenHeader`, which composes
`ResponsivePageHeader`, plus `PageHeaderMenu` — the hand-rolled hero headers on
...
| `ChoiceGroup` (`variant="inline"\|"card"`, single-select) | compact form choices render the shared sliding `TabBar` in `radiogroup` mode; explanatory card choices retain native radios. Replaces the three copy-pasted button rows, `VersionHistory`/`ProjectSettingsPage` pickers, and `AddWidgetPanel`'s tone picker. |
...
- This plan never edits `ScreenHeader`, `ResponsivePageHeader`,
