...
- **13** files use `admin-input-compact` (the 30px dense/inline variant) — `ScheduledTodoTemplate.tsx`, `TodoTemplateEditor.tsx`, `ExecutorCreatePanel.tsx`, `ToolFilterBar.tsx` (×2), `EventTriggerFields.tsx` (×2), `WebhookTriggerFields.tsx` (×4), `TriggerListColumn.tsx`, `CommentComposer.tsx`, `WorkspaceMemberPeople.tsx`.
