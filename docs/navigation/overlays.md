...
thought-process dialog, or a file in the ticket dialog's Attachments or its
comments, `useAttachmentViewer(token, { blocking: true })` — since two modals
tie on layer and Back and the one beneath would win). Each
