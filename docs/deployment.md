...
| UOA tariff/subscription boundary | `UOA_BILLING_APP_KEY_NESSIE`, `UOA_BILLING_ACTOR_PRIVATE_JWK_NESSIE`, `UOA_BASE_URL` | Nessie's own `uoa_app_` key bound in UOA to the `nessie` billing service, exact actor issuer, public half of the dedicated RS256 actor key, and `https://app.nessie.works` return origin. The app key and private JWK are separate GitHub Actions secrets installed atomically into the root-readable host `.env`; neither may be reused by Ledger or another product. Every summary/Checkout/Portal/cancel call carries a fresh 45-second actor JWT for the exact linked UOA user/org/team. |
