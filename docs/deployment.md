...
| Ledger DeepWater MCP endpoint | `LEDGER_DEEPWATER_MCP_URL` | Required to enable DeepWater for a team. Hosted production value: `https://ledger.unlikeotherai.com/v1/mcp/deepwater`. This must be Ledger's bearer-authenticated adapter, never the upstream provider. If unset, `PATCH .../deep-water/team-enablement` fails with `LEDGER_DEEPWATER_MCP_URL_UNSET` (503). The generated team instance has no shared token; each caller stores a dedicated Ledger ProxyToken through the encrypted existing-instance credential form, which creates a per-user override and keeps Ledger's job and booked rate-card charge ownership isolated. The booked charge is not a provider-invoice actual; complex runs may reconcile higher upstream. |
