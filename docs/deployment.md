...
| DeepWater MCP endpoint | `DEEP_WATER_MCP_URL` | Required to enable DeepWater for a team. The HTTP MCP URL the team-scoped `deep-water` instance is provisioned against. If unset, `PATCH .../deep-water/team-enablement` (enable) fails with `DEEP_WATER_MCP_URL_UNSET` (503) rather than creating a dead instance |
