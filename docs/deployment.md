...
| Public admin origin | `NESSIE_ADMIN_PUBLIC_URL` | `https://app.nessie.works` in production. Used only for server-selected Stripe Checkout success/cancel and billing-portal return URLs; callers cannot supply or widen it. |
...
| Ledger metering reader | `LEDGER_BILLING_READ_APP_KEY_NESSIE` | Nessie's dedicated, product-bound, read-only Ledger app key (`billingProduct=nessie`, `billingReadEnabled=true`, no provider grants). It must be different from `LEDGER_PROXY_TOKEN` and `NESSIE_MODEL_API_KEY`; Nessie has no fallback between them. The API exchanges a fresh UOA delegation for exact `billing.read` scope and proxies only immutable provider/API usage, cost, attribution, and snapshot identity. Legacy commercial fields in Ledger's compatibility response are ignored. |
| UOA commercial billing boundary | `UOA_BILLING_APP_KEY_NESSIE`, `UOA_BILLING_ACTOR_PRIVATE_JWK_NESSIE`, `UOA_BASE_URL` | Nessie's own `uoa_app_` key bound in UOA to the `nessie` billing service, exact actor issuer, public half of the dedicated RS256 actor key, and `https://app.nessie.works` return origin. The app key and private JWK are separate GitHub Actions secrets. The deploy runner cryptographically validates both before its dependency-free host installer atomically updates the root-readable `.env`; neither may be reused by Ledger or another product. Every summary/Checkout/Portal call carries a fresh 45-second actor JWT for the exact linked UOA user/org/team. Cancellation stays disabled until the UOA preview/confirm protocol is consumed. |
