# UOA team (company) avatars in Nessie (2026-07-25)

## Goal

Give a team the same treatment
...
The team is the team — the thing `TeamSwitcher` switches between. The
UOA team id is `Team.externalTeamId`, always resolved through the actor's
own organization for current-team operations. The current-team read
...
relay is called; this safely covers every organization in the user's team
list. A team with no `externalTeamId` (a local, non-UOA team) is a `404`,
...
team's picture should not have to have signed in through SSO this session.

### API

| Route | Who |
| --- | --- |
| `GET /api/team/avatar` | any authenticated actor in the team |
| `GET /api/teams/:teamId/avatar` | a signed-in user who belongs to that team |
| `PUT /api/team/avatar` | organisation **owner or admin** |
| `DELETE /api/team/avatar` | organisation **owner or admin** |
...
max-age=300`, cacheable `404` when there is no UOA team, `502` when the
...
- `TeamAvatar` primitive (`admin/src/components/primitives/`), sibling of
  `UserAvatar`: resolves either `/api/team/avatar` or the membership-scoped
  `/api/teams/:teamId/avatar` through the existing
  `useAuthedObjectUrlFromPath` bearer-fetch helper, and falls back to team
  initials. Square-ish (`rounded-xl`) to match how the team already
  renders, versus the round user avatar.
- The sidebar rail has one team identity control: the team-switcher
...
- `TeamAvatarPanel` on Settings → Organization → General, directly beside
...
The org logo and the team avatar are genuinely different things and the
panel says so: the logo is Nessie's own org-wide brand mark on the sign-in
screen, the team avatar is the company picture UOA holds for this team and
...
- [x] `api/src/routes/team-avatar.ts` — three relay routes + the owner/admin
      gate, registered in `api/src/index.ts`.
- [x] `TeamAvatar` primitive + sidebar team-switcher button.
- [x] Membership-scoped team-avatar reads for all team-switcher rows,
      duplicate rail branding removal, and viewport-safe menu placement.
- [x] `TeamAvatarPanel` on Organization → General + facade hooks.
- [x] `api/test/uoa-team-avatar.test.ts` — read/upload/remove happy paths,
...
precedence, `X-UOA-Avatar-Source`), with `Team.externalTeamId` bound on the
dev team. Everything below was then exercised through the running app and
removed afterwards.

- `GET` relayed to
  `…/domain/teams/<externalTeamId>/avatar?domain=…` with a
...
- With `UOA_*` unset (the local default) every relay answers a cacheable `404`
  and both surfaces show team initials with no layout change.
