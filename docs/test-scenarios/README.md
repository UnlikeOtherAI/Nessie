...
| 4 | `api/src/services/message-create.ts` | Legacy cross-channel `@mention` grabbed too many words        | Greedy regex `/@([\w][\w\s]*[\w]|[\w]+)/g` captured "Code Buddy what think" as one name                  | Composer mentions now carry exact agent ids; the per-agent name match remains only for legacy plain-text requests. |
