...
> **Two entries have fixes in flight as of 2026-07-23** — realtime
> channel-privacy fan-out (L2, `realtime-isolation` worktree) and silent
> budget-cap truncation (L1, `run-failure-surfacing` worktree + the Agent
> Harness v2 spec). Both are worded so they read correctly whether or not those
> branches have landed. If you find the cited symptom gone, treat the entry as
> Resolved.
...
|----|------------|---------------------|--------|----------------|
| L1 | **Silent budget-cap truncation.** When a per-run cap (iterations / tool calls / wallclock / tokens / cost) fires mid-run, the loop returns empty/partial `finalText` and the run still completes "successfully"; the only handler for exhaustion is a `console.warn`. | `worker/src/run/execute/agent-loop.ts:279` (`onBudgetExhausted` → `console.warn`); caps in `worker/src/run/agentic-loop.ts` (`EFFORT_BUDGETS`) | Fix in progress | Confirmed present: the exhaustion callback is still a bare `console.warn`. Caps are now effort-scaled (they were a single `DEFAULT_BUDGET`) but the silent-truncation behaviour remains. `docs/plans/2026-07-20-agent-harness-v2.md` (approved, unscheduled) calls this "the single worst UX failure mode" and specifies a visible `waiting_budget` pause; the `run-failure-surfacing` worktree is the in-flight fix. |
| L2 | **Realtime WS/SSE channel-privacy fan-out.** Historically events were published at org scope and delivered on org match alone, bypassing channel privacy. | `api/src/realtime/hub.ts` (`shouldDeliverWsNotification`); `api/src/lib/request-helpers.ts:433` (`canAccessChannelRealtimeEvent` via `getVisibleChannel`, `filterAuthorizedScopes`); wired at `api/src/index.ts:215` | Fix in progress | Real mitigation is now on `main`: delivery filters by channel scope and re-checks `getVisibleChannel`, and subscription registration runs through `filterAuthorizedScopes`. The remaining exposure is org-scoped notifications that carry no channel scope (they still fan out by org match). The `realtime-isolation` worktree is hardening this; word as partially mitigated until it lands. |
