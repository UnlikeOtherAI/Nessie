...
- An executor can offer the built-in `coding-sessions` bridge: Claude Code or
  ...
  each agent's turn budget, the live-session quota, folders, environment
  variable names, configuration digest — are in the signed descriptor and
  read on review as "Coding agents on this machine: Claude Code (accept
