...
| workspace (company) avatar | `GET/PUT/DELETE /api/workspace/avatar` — the company image UOA holds for the signed-in workspace, relayed to UOA `/domain/teams/{externalWorkspaceId}/avatar` with the domain-hash bearer (UOA precedence: uploaded → proxied team `iconUrl` → generated SVG). `PUT` takes multipart part `file` (PNG/JPEG/WebP ≤ 1 MiB, magic-byte validated by UOA); `DELETE` is idempotent. The team is resolved from the actor's **session** team through their own organisation and never from the request, so a caller can only address their own workspace; a team with no `externalWorkspaceId`, or unconfigured UOA, is a cacheable `404`. **The domain hash is full system trust and UOA's `/domain/*` mutations apply no role check — Nessie's owner/admin gate on `PUT`/`DELETE` is the only authorization.** Same relay hardening as the user avatar. No new environment variables | N/A | N/A (Settings → Organization → General → Workspace avatar; rendered in the sidebar workspace switcher) | read: any actor in the workspace; write: organisation owner/admin | no | no | implemented (REST admin surface) |
