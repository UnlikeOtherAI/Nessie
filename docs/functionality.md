...
| team invitations (UOA) | `GET /api/team/invitations` (UOA `GET .../teams/{teamId}/invitations`), `POST /api/team/invitations` (`{invites:[{email,name?,teamRole?}]}` → UOA's trusted-backend bulk invite; the response carries UOA's per-email verdict `invited\|resent_existing\|already_member\|existing_user\|conflict`), `POST /api/team/invitations/{inviteId}/resend` (refreshes UOA's 30-day expiry), `POST /api/team/invitations/{inviteId}/revoke` (withdraws an invitation that was already sent → UOA `DELETE .../teams/{teamId}/invitations/{inviteId}`; **idempotent** — revoking twice is still `200`, an invitation that has already been accepted is `409 INVITATION_ALREADY_ACCEPTED` because removing the member is a different decision, an unknown or foreign invite id is a generic `404`, and a `200` whose body is not `{ok:true}` is `502 UOA_DIRECTORY_UNAVAILABLE` rather than a silent revoke), `POST /api/team/invitations/{inviteId}/approve\|deny` (UOA's member-initiated-invite review queue; deny is the stop verb for an invitation that was never sent, revoke for one that was). **Acceptance is hosted by UOA**: Nessie never mints, stores, or renders an invitation token, and has no accept flow. The signed-subject member form delegates one actionable invitation per exact team and normalized email to UOA: a repeated submission is an upstream resend, not a local duplicate. Invite emails are PII, so the list is owner/admin only — matching UOA's own gate. Shareable invite links (`.../invite-links`) are deliberately not surfaced | N/A | N/A (Settings → Organization → Members → Invite to team / Pending invitations) | organisation owner/admin | no | no | implemented (REST admin surface) |
