...
| user, agent, and channel favourites | `GET /api/favorites`, `PUT/DELETE /api/favorites/{targetType}/{targetId}` | N/A | N/A (star beside the admin chat title) | actor's org + visible channel/person/agent target | no | no | implemented (REST admin surface; the chat title star persists per signed-in user for channels, one-to-one DMs including self-DM, and the Personal Assistant agent) |
