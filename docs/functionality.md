...
| customer billing usage | `GET /api/ledger/billing/usage?month=YYYY-MM&groupBy=service\|team\|user` | N/A | Usage & billing | active-team owner/admin; exact UOA workspace match | no | yes | implemented (server-only dedicated Nessie billing-reader app key + fresh `billing.read` UOA delegation; additive CC0 Ledger schema v4; raw provider units/cost, billable equivalent units, base, added value, calculated customer charge, Stripe eligibility/collection mode, billing/caller/origin products, monthly tariff/subscription terms, and immutable snapshot shown separately) |
| customer subscription lifecycle | `GET /api/billing/subscription`, `POST /api/billing/checkout`, `POST /api/billing/portal`, `POST /api/billing/subscription/cancel` | N/A | Usage & billing | active-team owner/admin; exact UOA workspace and UOA membership re-check | checkout, portal, cancel: yes | yes | implemented (Nessie has no tariff or payment store; its dedicated UOA app key plus a separate 45-second RS256 actor assertion proxy content-free tariff/subscription state and fixed-origin Stripe Checkout/Portal actions to the authoritative SSO) |
