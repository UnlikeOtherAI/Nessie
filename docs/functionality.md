...
Current addition (2026-09-21): **Admin › Automations › Batch jobs** and Documents' processing
...
[Batch jobs (Task Sets)](standards/task-sets.md).
...
- Activation reuses the user's existing linked UOA subject and active team;
...
- Owners manage individual explicit entries on `/admin/advanced/tools` through
...
- UOA supplies the complete display-ready statement, shared-team credits,
  ...
  linked team, validates the shared public protocol, renders it, and
  proxies its frozen actions.
- API validation and admin view-model types come directly from the public
  MIT-licensed `@unlikeotherai/billing-statement-protocol` 1.2.0 team
...
- Exact UOA Checkout return markers on `/` preserve their full query while
  routing to Credits and billing. That customer surface contains only UOA-authored
  ...
  owner-only `/admin/usage` page. The credits page displays a neutral notice and
...
### 2.0c Live personal-assistant team provisioning boundary

- Four PA-only builtins let the assistant do to an existing team what its
...
- The admin **Agents page** (`/admin/agents`) groups the same entitlement-scoped list
...
- `agent_list` and `channel_create` (`POST /api/channels`) are open to any
  ...
  `launchOrigin` (creator + UOA team) on scheduled and interval triggers;
...
- The shared implementations live in `@nessie/team-admin`, consumed by both
...
- The database keeps standalone channels in one hidden, system-managed channel
  ...
  organisation or team hierarchy, and it is excluded from project APIs and
  navigation.
...
- Only a **private** executor's **pairing owner** drives it: the API refuses
  ...
  `coding_session_list`, `_start`, `_wait`, `_send`, `_interrupt`, `_review`
  and `_close`; the agent briefs, waits, reviews and closes, and never writes
  the code itself.
- Sessions outlive runs. A lease's end, the agent's or the owner's access
  withdrawn, a paused or revoked machine, and the owner's own Close each write
  a close request, which the machine's next heartbeat carries until a report
  shows it done.
- The executor page's **Permissions** tab lists the open sessions under the
  bridge's status in **Local apps** — title, status, coding agent, folder,
  the agent driving it (named only when the reader could see that agent) and
  last update — for the people who manage the machine, with **Close** for the
  pairing owner only; a closed row reads "Closing…" until the machine's next
  report drops it. The contract is
  [docs/executor-protocol/host-coding-sessions.md](executor-protocol/host-coding-sessions.md).

### 2.0h Screenshots from local apps

- An image a local program returns (a Kelpie screenshot) leaves the result on
  the machine, is uploaded before the receipt and kept as a `FileService` file
  of its executor command; the model reads it through the one prompt-image
  loader ([docs/standards/file-storage.md](standards/file-storage.md)).
- A person sees it where they read the call: as a thumbnail under the call's
  line in the **thought-process dialog** (the thinking bubble's doorway) and on
  the call's card in the agent page's **Activity → Tool execution log** (the
  home). A press opens the original in the attachment viewer. Only someone the
  attachment routes would serve it to is shown it at all; a reader who may see
  the call but not its image sees the call alone. The rules are in
...
| conversation start | `POST /api/channels/conversations` then `POST /api/threads/{threadId}/messages` | N/A | N/A (sidebar compose icon -> `/channels/new`; agent `@mentions` -> their direct-message conversation) | organization membership; agent recipients require owner | no | no | implemented (the recipient-addressed composer opens as a focused full-screen task with Close on phone and an accessible modal sheet over the retained Channels team on tablet/desktop; Close returns to the initiating internal route or `/channels` for a direct link, while a successful send replaces the composer with the thread the first message was written in — never an agent DM's bare address, which is its session home. That first message is written in the one chat composer, so a pasted screenshot or copied image, the paperclip's files and files dropped on the sheet are staged with upload progress and linked by it, with text, files or both; Enter waits while an upload is in flight, and a start that fails keeps the words and the staged files for another try. A credential in that first message is stopped before the conversation is even started, as in every composer: the vault capture form opens with Personal scope, since there is no room yet whose project it could offer. Save starts the conversation with the masked text and the protection notice, and Discard sends nothing and starts no conversation; the staged files stay for the next send either way. An agent `@mention` resolves or creates its one-to-one DM and opens it ready for a message. The composer autocompletes people and agents, marks agents with a shared `Team` or lock-bearing `Private` chip so duplicate names remain distinguishable, opens a single-user DM or single-agent DM for one recipient, and creates a stable group DM for mixed or multi-recipient chats after the first message. Group DMs appear in Direct messages, never in Projects or Channels.) |
...
| agent thought process (thinking bubbles) | `GET /api/threads/{threadId}/thinking` (live runs of the thread with their reply anchor, tail log, and `lastChunkId`), `GET /api/threads/{threadId}/runs/{runId}/thinking` (full log: last 500 entries with a `truncated` flag) + realtime `stream.start` (`rootMessageId`), `stream.reasoning`, `stream.thinking.tool` | N/A | N/A (dashed thinking bubble at the bottom of the surface the reply will land on; tap/click opens the full thought-process dialog, where a tool line whose call returned a local program's images shows them as thumbnails that open full size; its Stop icon is the run-stop row below) | same gate as the thread SSE stream (thread/channel visibility); the run must belong to that thread | no | no | implemented (backend; the worker's per-run thought recorder persists coalesced visible reasoning and one line per tool call to `run_thinking_chunks` and publishes each flush with its chunk id, so the live bubble is lossy but the record is not; a tool line names the `ToolCall` it became once the call ends (`tool_call_id`), and the full log carries that call's screenshot refs for a viewer the attachment routes would serve them to, which is why the dialog reads it on open and again as each call returns; `stream.*` is never replayed from the SSE backlog, so mid-run joiners bootstrap over REST and dedupe by `chunkId`; chunk ids are BigInt columns serialized as strings) |
| run stop | `POST /api/runs/{runId}/cancel` (a queued or suspended run is `cancelled` at once; a running run gets a cooperative flag the loop reads between iterations and after a tool batch) | N/A | N/A (a **Stop** icon on every thinking bubble — the channel tail and the compact one under a thread root — and beside the agent page's status pill; pressed it reads "Stopping…" until the run leaves its live state; it never adds a line to the composer) | the caller must be able to reach the run; a DeepWater handoff run refuses with `409 RUN_HANDOFF_MANAGED` | no | no (the `run.cancelled` TaskEvent records who stopped it) | implemented (`admin/src/components/shared/RunStopButton.tsx` over `useCancelRun`; drawn only while its surface knows the run is live — a bubble from `stream.start` to `stream.done`, the agent header while its status read names a `currentRunId`, which it does for a pending, running, `waiting_approval` or `waiting_input` run, so a suspended run is stopped from the agent header; a refusal restores Stop with the server's reason; `pnpm --filter @nessie/admin test:e2e:run-stop` pins the button, the pending state, the request and both suspended states) |
| executor run launcher | `POST /api/executor-availability` (opaque, scope-only candidates for one bundle) then `POST /api/threads/{threadId}/executor-runs` (the human message, run, task and every binding in one transaction) | N/A | N/A (composer → **Run on an executor**: pick a bound channel agent and one of eight bundles — list, read, write, or write and review workspace files; act in an approved site; run and review a workspace command; a managed Codex session; **Local apps on this machine** (`mcp.tools` + `mcp.call`, the programs the owner named in the reviewed policy)) | a person only; each operation re-authorized and bound at commit, never an executor id on the wire | no | no (each executor command is a ToolCall with receipts; `browser.act` and `command.run` also emit audit events) | implemented (candidates stay opaque: the dialog never learns program names or an executor label, and with no candidate it renders the resolver's explanation; `pnpm --filter @nessie/admin test:e2e:executor-run-launcher` pins the eight options, the local-apps description and the posted payload) |
| executor coding sessions | `GET /api/executors/{executorId}/coding-sessions` (the open sessions the machine last reported, with `closing`, the driving agent's name where the reader may see it, and `canClose`), `POST /api/executors/{executorId}/coding-sessions/close` (`{ownerKey, sessionId}`, 202) | N/A | N/A (executor page → **Permissions** → **Local apps** → the coding bridge's session list, with **Close** for the pairing owner) | people who may manage the machine read; only its pairing owner closes, and only a session the report lists as theirs | no | yes (`executor.coding_session.close_requested`) | implemented (Close writes a `person` close request the next heartbeat carries; the row reads "Closing…" until a later report drops it; `pnpm --filter @nessie/admin test:e2e:executor-coding-sessions` pins the list, Close, the administrator without Close and the phone width) |
| model-judged reply placement | `POST /api/threads/{threadId}/messages` (orchestration side effect; replayed by `POST /api/runs/{runId}/restart`) | N/A | N/A (reply lands in the trigger's reply thread or in the main channel window) | channel membership | no | no | implemented (the engagement-decision model call also returns `replyPlacement` — `thread` when the answer belongs to the asker's exchange, `channel` when it is a standalone contribution to the room, defaulting to `thread` — stored on `Run.replyPlacement`; @mentions stamp `thread` and system DMs `channel` structurally with no model call; a DM whose only member is the person talking answers in its main chat, and with one agent in it Jev decides before the run whether the answer is a reply, the work done and marked ✅, or a reaction, and whether it goes back to an earlier message — mentioned, linked (`metadata.messageRef`) or answered under it as a thread (docs/standards/reply-threads.md → "One-on-one rooms"); resolution precedence is DeepWater handoff/external agent → top-level, then an already-in-thread trigger → that root, then Jev's earlier-message thread, then a `channel` judgement → top-level, else the trigger message; the resolved anchor is persisted on `Run.replyRootMessageId` for REST readers) |
...
| message agent profile + quick mention | `GET/POST /api/threads/{threadId}/messages` with structured `agentMentions` | N/A | N/A (tap/click an agent-authored message avatar or name) | channel membership + agent visibility | no | no | implemented (admin chat opens a right-side agent info drawer that reuses the channel feed/composer for the current thread, filters to the selected agent exchange, and addresses quick prompts with the agent's exact id-keyed mention; the mention typeahead labels agents with the same shared `Team` / lock-bearing `Private` chip as the new-message address book; disabled in the Personal Assistant DM) |
...
| project lifecycle | `GET/POST /api/projects`, `GET/PATCH/DELETE /api/projects/{projectId}` | `project.create`, `project.update`, `project.delete` | `/project create` | org + project policy (delete: owner, refused while the project still has channels, knowledge spaces/pages, executors, or a UOA-bound team) | yes for unsafe transitions | yes | implemented (REST admin surface) |
...
| team (company) avatar | `GET/PUT/DELETE /api/team/avatar` plus read-only `GET /api/teams/{teamId}/avatar` — the company image UOA holds for a team. The active UOA team uses `/org/organisations/{externalOrgId}/teams/{externalTeamId}/avatar` with the domain hash and a short-lived subject assertion; UOA requires the asserted team to match that route. A caller with no matching assertable session — including a picker read for another entitled team — uses `/domain/teams/{externalTeamId}/avatar` with the domain-hash bearer (UOA precedence: uploaded → proxied team `iconUrl` → generated SVG). `PUT` takes multipart part `file` (PNG/JPEG/WebP ≤ 1 MiB, magic-byte validated by UOA); `DELETE` is idempotent. Current-team reads and mutations resolve the actor's **session** team through their organisation and accept no team id. UOA's authenticated `/org/me` directory supplies a public, always-resolving `avatarImageUrl` for every team the caller may enter; UOA's public image route is rate-limited and non-enumerating, and Nessie resolves relative URLs against the configured UOA origin before storing them. `/auth/me` also reconstructs that public URL from the external team id for sessions created before UOA added the directory field, and pins `size=128` so clients do not reuse a cached response from before cross-origin embedding was enabled. The switcher uses the existing membership-scoped relay where a local `avatarTeamId` exists (preserving immediate cache-busting after edits), otherwise the public UOA URL, and finally initials. It groups teams under their owning organisation by stable organisation id on desktop, iPad, and phone. **The domain hash is full system trust and UOA's `/domain/*` mutations apply no role check — Nessie's owner/admin gate on `PUT`/`DELETE` is the only authorization.** Same relay hardening as the user avatar. No new environment variables | N/A | N/A (Admin › Teams → the team → General → Team avatar; the shared team switcher shows every authorized UOA picture under its organisation) | read: current team actor or signed-in member of the requested team; write: organisation owner/admin | no | no | implemented (REST admin surface) |
| team roster (UOA) | `GET /api/team/members` — one page of the actor's UOA team roster, read live from `GET /org/organisations/{externalOrgId}/teams/{externalTeamId}/members` with `status`, `limit`, `cursor`, and `direction`. That exact-team response owns each person's UOA subject, display name, email, team role, lifecycle status, avatar URL, page metadata, and caller permissions. Nothing is persisted: UOA owns human identity and membership. Members are named by their **UOA subject**, never a local user id or an email row. Calls run in UOA **signed-subject mode** — the domain-hash bearer plus an `X-UOA-Subject-Assertion` carrying a one-minute RS256 assertion of the signed-in UOA subject and their active org/team (UOA §4.6c), which UOA verifies through Nessie's published config JWKS and then re-resolves the credential epoch and live access to the exact target team before applying its own role gate and user audit attribution. `X-UOA-Access-Token` is never sent — Nessie holds no spendable user credential — and a caller with no current UOA session is refused `403 UOA_SESSION_REQUIRED` rather than falling back to tenant-wide backend mode. A team with no `externalTeamId`/`externalOrgId`, or a deployment without UOA, answers `404 TEAM_NOT_LINKED` — there is never a fall back to local rows. No new environment variables | N/A | N/A (Admin › People, on a UOA session) | any member of the team | no | no | implemented (REST admin surface) |
| team member avatar (UOA) | `GET /api/team/members/{uoaSub}/avatar` — the picture UOA holds for one person in the actor's team roster, relayed from UOA `GET /domain/users/{uoaSub}/avatar` with the domain-hash bearer. It exists because a roster row is named only by a UOA subject: the organisation-scoped relay `GET /api/users/{userId}/avatar` is keyed on a Nessie user id, and a roster member may have no local row at all. **The requested subject is checked against the team's own roster before any avatar call**, using the same `GET /api/team/members` read (briefly cached in-memory per team, ~30 s, so one page load asks UOA once rather than once per row) — without that check the full-trust domain-hash path would hand any member the picture of anybody in the whole UOA domain. A subject outside the roster and a subject UOA has no image for are the same cacheable `404 AVATAR_NOT_FOUND`, so nothing is learned about foreign subjects; an unlinked team or a deployment without UOA is `404 TEAM_NOT_LINKED`; an unreadable roster is `502 UOA_DIRECTORY_UNAVAILABLE` and an unreachable avatar endpoint `502 UOA_AVATAR_UNAVAILABLE` — never a silent miss. Same relay hardening as the other avatar routes (content-type allowlist, `nosniff`, `content-security-policy: default-src 'none'`, `cache-control: private, max-age=300` on hit and miss alike). The roster renders it through the shared `UserAvatar`, falling back to initials. No new environment variables | N/A | N/A (Admin › People, on a UOA session) | any member of the team (identical to the roster read) | no | no | implemented (REST admin surface) |
| team membership mutations (UOA) | `PUT /api/team/members/{uoaSub}/role` (`{role: admin\|member}` → UOA `PUT .../teams/{teamId}/members/{userId}` `{team_role}`), `DELETE /api/team/members/{uoaSub}` (UOA soft-remove + team session revocation), `POST /api/team/members/{uoaSub}/deactivate\|reactivate` (organisation-level in UOA; UOA refuses to deactivate an owner). **Backend mode has no acting user, so UOA applies no role check and records `actor_user_id: null` / `uoa_actor.via = domain_backend` — Nessie's owner/admin gate is the only authorization**, the same rule as the team avatar relay. Upstream 4xx → `TEAM_MEMBERS_REJECTED`, transport/5xx/malformed body → `502 UOA_DIRECTORY_UNAVAILABLE` | N/A | N/A (Admin › People) | organisation owner/admin | no | no | implemented (REST admin surface) |
| automatic team access by verified email domain | `GET/PUT /api/organization/automatic-membership[...]` (claim, DNS verify, rotate challenge, suspend/resume, revoke, team set, re-authorize, reconcile, cancel, emergency stop) and `GET/PUT /api/team/automatic-membership[...]` (this team only, no challenge). A grant is a relay to UOA `addTeamMember` carrying a fresh 60-second org-scoped subject assertion for the administrator who authorized the rule, so **UOA re-resolves their live role on every call** — never backend mode, and never a local membership write. No call names a role (UOA's add is becoming an upsert, so naming one would demote an existing team owner), and membership is read first so an existing member is skipped entirely. | N/A | N/A (Admin › People → **Automatic team access**, at organisation and team scope; one shared panel, plus a doorway in the invitation dialog) | organisation owner/admin for domains and the emergency stop; team owner/admin for their own team's rule only | no | no | implemented behind `NESSIE_AUTOMATIC_MEMBERSHIP_ENABLED` (default off). DNS TXT proof needs two observations ten minutes apart before a claim takes the instance-wide exclusivity lock; consumer/disposable providers, IP literals, localhost and public suffixes are refused; matching is exact, so a subdomain needs its own claim. Narrowing, suspending, revoking, losing DNS or losing authorization stops **future** grants only — nobody is ever removed. Reconciliation is a durable cursor-based job that re-mints its principal every batch, is rate-limited, supersedable and cancellable, and reports counters only, never a list of matching people. |
| team invitations (UOA) | `GET /api/team/invitations` (UOA `GET .../teams/{teamId}/invitations`), `POST /api/team/invitations` (`{email,name?,teamRole?}` → UOA's user-mode single invite; one address per request, and the older `{invites:[…]}` bulk envelope is no longer accepted), `POST /api/team/invitations/{inviteId}/resend` (refreshes UOA's 30-day expiry), `POST /api/team/invitations/{inviteId}/revoke` (withdraws an invitation that was already sent → UOA `DELETE .../teams/{teamId}/invitations/{inviteId}`; **idempotent** — revoking twice is still `200`, an invitation that has already been accepted is `409 INVITATION_ALREADY_ACCEPTED` because removing the member is a different decision, an unknown or foreign invite id is a generic `404`, and a `200` whose body is not `{ok:true}` is `502 UOA_DIRECTORY_UNAVAILABLE` rather than a silent revoke), `POST /api/team/invitations/{inviteId}/approve\|deny` (UOA's member-initiated-invite review queue; deny is the stop verb for an invitation that was never sent, revoke for one that was). **Acceptance is hosted by UOA**: Nessie never mints, stores, or renders an invitation token, and has no accept flow. Create and resend both send `redirectUrl` = `UOA_REDIRECT_URL` (`<admin>/login`), added server-side in `packages/team-admin` and never accepted from the client, so UOA's terminal page can offer a way back into Nessie instead of ending on "You can close this window". The signed-subject member form delegates one actionable invitation per exact team and normalized email to UOA: a repeated submission is an upstream resend, not a local duplicate. Invite emails are PII, so the list is owner/admin only — matching UOA's own gate. Shareable invite links (`.../invite-links`) are deliberately not surfaced | N/A | N/A (Admin › People → Invite to team / Pending invitations) | organisation owner/admin | no | no | implemented (REST admin surface) |
| agent avatar | `POST /api/agents` accepts an optional uploaded `avatarAttachmentId`; when omitted it creates a prompt from the agent's name/role/purpose and sends it through Ledger to OpenAI `gpt-image-2`, storing the resulting PNG through the standard FileService attachment path (local filesystem now, S3 when configured). New-user provisioning also bootstraps the organization-managed Personal Assistant's private DM and creates that assistant's first original, role-appropriate headshot; later users keep the established assistant avatar. A transient prompt/image failure never blocks account provisioning or sign-in; the next sign-in or PA bootstrap retries while no avatar is stored. `POST /api/agents/{agentId}/avatar/generate` creates a private preview, and the owner explicitly confirms it through `PATCH /api/agents/{agentId}/avatar`; uploads use `POST /api/uploads`. The generate body also accepts optional free-text `instructions` — the person's own description of the look they want ("a friendly robot in a hard hat") — which the prompt writer honours within the fixed safety constraints (single original subject, no text/logos, the exact pastel background); with no `instructions` the model defaults to a fictional human headshot, and selects a machine only where the role and purpose establish a non-human machine. Records expose both the attachment id and a persisted random pastel background colour, rendered behind every profile headshot. Editing an agent surfaces one avatar card whose pencil opens a single modal — a larger preview, an optional prompt with **Generate with AI**, **Upload** (crop), **Remove image**, and a top-right close — the same `AgentAvatarQuickEdit` the detail header uses. | N/A | N/A (Admin › Agents → open agent → Edit tab → avatar pencil → prompt + Generate with AI / Upload / Remove; Admin › Agents → New agent → Agent avatar; Direct messages → Personal Assistant) | creation: authenticated actor; generation/replacement: owner with exact-org agent visibility; attachment: actor-visible same-org image | no | no | implemented (REST admin surface; the avatar pencil is only rendered for owners, while the server repeats the owner check and shows an in-place spinner while generation is pending) |
| instance announcements and News | `GET /api/announcements/banner`, `GET /api/news`, `POST /api/news/read`, `PATCH /api/news/preferences`; super-admin editing under `/api/platform/announcements`; authenticated article images at `/api/news/{id}/image` | N/A | N/A (Advanced → Announcements; account menu → News) | all signed-in human accounts read; instance super-admin writes; each account owns its News read marker and mute setting | no | no | implemented (strip dismissal stays on the device; News opens a phone page or wider-layout dialog; see [announcements](announcements.md)) |
...
- `team` is a legacy alias for `project`; all new APIs and docs must use `project`, and legacy input must be canonicalized before policy evaluation.
...
- Resolves must pass policy checks on actor binding + scope + project + role and emit auditable deny reasons.
  - policy chain: org -> project -> team -> channel -> explicit secret bindings, deny-first at each layer. (That order follows the schema's current `Team.projectId` direction, which is inverted relative to the model in `docs/standards/team-model.md`; it is the resolution order the code uses today, not the hierarchy.)
...
- User status contact rules are persisted and editable in `/settings/status`, but inbound message dispatch does not yet evaluate them to start response agents.
