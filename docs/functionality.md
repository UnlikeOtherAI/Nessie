...
| customer usage metering | `GET /api/ledger/billing/usage?month=YYYY-MM&groupBy=service\|team\|user` | N/A | Usage & billing | active-team owner/admin; exact UOA workspace match | no | yes | implemented-partial (server-only dedicated Nessie metering-reader app key + fresh `billing.read` UOA delegation; the compatibility Ledger schema-v4 response is accepted, but Nessie renders only immutable raw provider/API units, estimated/actual provider cost, billing/caller/origin attribution, and snapshot identity; every Ledger commercial field is ignored while migration to the raw-only metering endpoint proceeds) |
| customer commercial billing surface | `GET /api/billing/subscription`, `POST /api/billing/checkout`, `POST /api/billing/portal` | N/A | Usage & billing | active-team owner/admin; exact UOA workspace and UOA membership re-check | checkout, portal: yes | yes | implemented-partial (Nessie has no tariff, rating, payment, or cancellation-decision logic; its dedicated UOA app key plus a separate 45-second RS256 actor assertion proxy UOA-owned summary and fixed-origin Stripe Checkout/Portal actions. Direct cancellation is disabled until Nessie can render UOA's versioned preview/confirm action model, including UOA's cross-service direct-access decision.) |
