...
| UOA automatic-membership control bridge | `NESSIE_UOA_AUTOMATIC_MEMBERSHIP_CONTROL_SECRET` | Optional shared secret held only by the UOA control-plane service and Nessie. When unset, `POST /api/internal/uoa/automatic-membership/control` is fail-closed. When set, UOA signs each JSON request with `x-uoa-automatic-membership-timestamp` (Unix milliseconds), `x-uoa-automatic-membership-request-id` (the body `request_id` UUID), and `x-uoa-automatic-membership-signature` (hex HMAC-SHA256 of `${timestamp}.${JSON.stringify(parsedBody)}`). Nessie accepts a 60-second clock window and keeps a five-minute durable replay ledger. This route is server-to-server only; it never accepts a browser session or a local membership/profile as authority. |
