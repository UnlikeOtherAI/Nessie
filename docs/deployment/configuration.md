...
| Landing team list | `NESSIE_LANDING_ORIGIN` | `https://nessie.works` — the one origin `GET /api/auth/landing-teams` answers; deliberately not in `NESSIE_CORS_ORIGINS`. Unset: the landing shows no "Your teams" section ([team-hosts.md](../standards/team-hosts.md)) |
