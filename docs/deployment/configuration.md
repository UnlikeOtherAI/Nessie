...
| Landing team list | `NESSIE_LANDING_ORIGIN` | `https://nessie.works,https://www.nessie.works` — the exact origins `GET /api/auth/landing-teams` answers, comma-separated like `NESSIE_CORS_ORIGINS` (trimmed, empties dropped). Each entry must be a bare `http(s)` origin; a path, query, fragment, credentials or an unparseable entry fails config load. Deliberately not in `NESSIE_CORS_ORIGINS`. Unset: the landing shows no "Your teams" section ([team-hosts.md](../standards/team-hosts.md)) |
...
| DeepWater research events | `DEEPWATER_EVENTS_SECRET` | Optional. The HMAC-SHA256 key DeepWater signs its research events to Nessie with — DeepWater holds the same value as `NESSIE_EVENTS_SIGNING_SECRET` — 32 to 128 URL-safe characters, distinct from every other key. Installed into the production host `.env` from the GitHub Actions secret of the same name by `infrastructure/compose/set-deepwater-events-secret.sh`, which removes it when the secret is unset; an absent secret never fails the deploy. Unset (or shorter than 32 characters), `POST /api/integrations/deep-water/events` answers 503 `DEEP_WATER_EVENTS_UNCONFIGURED`, DeepWater keeps retrying for up to seven days, and the watch through Ledger serves alone. [Standard](../standards/deepwater.md) |
...
| Agent avatar images | `NESSIE_LEDGER_IMAGE_PURPOSE_API_ID` | Optional. When set, agent-avatar "Generate with AI" routes image generation through this Ledger **Purpose API** (`/v1/purpose/:id/images/generations`) on `NESSIE_MODEL_BASE_URL`'s Ledger host, so Ledger owns the image provider fallback chain (e.g. Gemini image primary, OpenAI `gpt-image-2` fallback) behind one endpoint. Unset keeps the direct `/v1/openai/images/generations` service route, which fails when OpenAI's key is exhausted. Uses the same `NESSIE_MODEL_API_KEY` bearer and signed identity as chat; the token must hold a grant for that Purpose API. **A portrait that fails says why:** the error names which of the two routes was asked (`Ledger purpose image route <id>` or `Ledger OpenAI service image route`), the HTTP status, Ledger's own error code and a redacted one-line excerpt of its response body, and a run out of time reads as a timeout rather than a refusal. Creation never fails over a picture, so the reason is logged instead: `request.log.warn` on `POST /api/agents` and `[worker.agent-avatar]` in the worker for an agent made in chat. |
...
| Signed-URL downloads | `NESSIE_STORAGE_PUBLIC_ENDPOINT`, `NESSIE_STORAGE_SIGNED_DOWNLOAD_MIN_BYTES` | **Unset on this deployment, deliberately.** Set the first and a download over the second (default 8 MiB) is answered as a `302` to a short-lived signed URL instead of being piped through the API, which is what otherwise pins a multi-GiB transfer to one instance. It is the store's address *as a browser sees it* and is not derived from `NESSIE_STORAGE_ENDPOINT`: MinIO here is `nessie-minio:9000` on a private Docker network with no ingress, so a signed URL naming it would be unreachable. Setting it also asserts the address answers with an `Access-Control-Allow-Origin` covering the admin origin, because the admin follows the redirect from `fetch()`. Unset, every download is proxied exactly as before |
