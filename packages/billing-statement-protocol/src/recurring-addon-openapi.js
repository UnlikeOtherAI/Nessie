...
import { billingSubjectActionBodySchema, billingSubjectRequestJsonSchema, } from './funding-schema-primitives.js';
import { billingRecurringAddonCancellationConfirmationV1JsonSchema, billingRecurringAddonCancellationConfirmRequestV1JsonSchema, billingRecurringAddonCancellationPreviewV1JsonSchema, billingRecurringAddonProtocolV1JsonSchema, billingRecurringAddonsV1JsonSchema, } from './recurring-addon-schema.js';
import { BILLING_RECURRING_ADDONS_CANCELLATION_CONFIRM_PATH, BILLING_RECURRING_ADDONS_CANCELLATION_PREVIEW_PATH, BILLING_RECURRING_ADDONS_CHECKOUT_PATH, BILLING_RECURRING_ADDONS_PROTOCOL_VERSION, BILLING_RECURRING_ADDONS_READ_PATH, } from './recurring-addon-types.js';
...
function selectorBody(selector) {
    return {
        ...
        content: {
            'application/json': {
                schema: billingSubjectActionBodySchema({ [selector]: { type: 'string', minLength: 1, maxLength: 256 } }, [selector]),
            },
        },
    };
}
...
export const billingRecurringAddonV1OpenApiDocument = {
    ...
    info: {
        ...
        description: 'Exact display-ready recurring add-on API. UOA resolves visibility, manager capabilities, paid entitlement, and cancellation; products only render and relay frozen actions.',
    },
    paths: {
        [BILLING_RECURRING_ADDONS_READ_PATH]: {
            post: {
                ...
                description: 'The product posts the exact subject bound into X-UOA-Actor. Active exact-team members receive a UOA-filtered display projection. Only billing managers receive enabled subscribe or cancel actions.',
                requestBody: {
                    required: true,
                    content: {
        ...
        [BILLING_RECURRING_ADDONS_CHECKOUT_PATH]: {
            post: {
                ...
                description: 'Products re-fetch the projection and relay the complete frozen action body unchanged. UOA verifies its subject against the app key and actor, then derives idempotency from the exact scope and offer.',
        ...
        [BILLING_RECURRING_ADDONS_CANCELLATION_CONFIRM_PATH]: {
            post: {
                ...
                description: 'Replay-safe through the opaque UOA preview token and UOA-issued idempotency key returned by the preview.',
    ...
    components: {
        ...
        schemas: {
            ...
            BillingRecurringAddonCancellationPreviewV1: billingRecurringAddonCancellationPreviewV1JsonSchema,
            BillingRecurringAddonCancellationConfirmRequestV1: billingRecurringAddonCancellationConfirmRequestV1JsonSchema,
            BillingRecurringAddonCancellationConfirmationV1: billingRecurringAddonCancellationConfirmationV1JsonSchema,
