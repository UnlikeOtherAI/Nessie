/**
 * The single JMESPath evaluator for workflow predicates and transforms
 * (workflows-first-class plan §5, carried by W16 `when:` and reused by W17's
 * `transform` step).
 *
 * Save-time compilation (`validateWorkflowGraphSteps`), the worker's `when:`
 * guard, and the future transform step all consume this module, so the
 * grammar and its security envelope are defined exactly once:
 *
...
export const WORKFLOW_JMESPATH_INPUT_MAX_BYTES = 1024 * 1024
export const WORKFLOW_JMESPATH_OUTPUT_MAX_BYTES = 256 * 1024
...
export const WORKFLOW_JMESPATH_EVAL_TIMEOUT_MS = 1_000

export type WorkflowJmespathResult =
...
const expressionSizeError = (expression: string): string | null =>
  byteLength(expression) > WORKFLOW_JMESPATH_EXPRESSION_MAX_BYTES
    ? `expression exceeds ${WORKFLOW_JMESPATH_EXPRESSION_MAX_BYTES} bytes`
    : null
 ...
 */
export const compileWorkflowJmespath = (expression: string): string | null => {
  if (!expression.trim()) {
...
const evaluateInWorker = (
  ...
  timeoutMs: number,
): Promise<WorkflowJmespathResult> =>
  new Promise((resolve) => {
    let settled = false
    const finish = (result: WorkflowJmespathResult): void => {
      if (!settled) {
        settled = true
        resolve(result)
      }
 ...
 */
export const evaluateWorkflowJmespath = async (
  expression: string,
  document: unknown,
  ...
  const serializedDocument = serializeCapped(document, WORKFLOW_JMESPATH_INPUT_MAX_BYTES)
  if (serializedDocument === null) {
    return {
      ok: false,
      error: `input document exceeds ${WORKFLOW_JMESPATH_INPUT_MAX_BYTES} bytes`,
    }
  }
  ...
  const evaluated = await evaluateInWorker(
    expression,
    serializedDocument,
    options.timeoutMs ?? WORKFLOW_JMESPATH_EVAL_TIMEOUT_MS,
  )
  if (!evaluated.ok) {
    return evaluated
  }

  const serializedOutput = serializeCapped(evaluated.value, WORKFLOW_JMESPATH_OUTPUT_MAX_BYTES)
  if (serializedOutput === null) {
    return {
      ok: false,
      error: `result exceeds ${WORKFLOW_JMESPATH_OUTPUT_MAX_BYTES} bytes`,
    }
  }
 ...
 */
export const isWorkflowJmespathTruthy = (value: unknown): boolean => {
