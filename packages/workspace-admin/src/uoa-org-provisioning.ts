...
/**
 * Creating UOA organisations and workspaces from inside the product, so that
 ...
 * in an authenticated switch response (`materializeUoaWorkspace`). A function
 * in this file therefore returns UOA ids and nothing else — there is
 * deliberately no place to put a local organisation.
 */

/** The UOA workspace a caller may now switch onto. Both ids come from UOA. */
export type UoaProvisionedWorkspace = {
...
const resolveDefaultTeamId = async (
  ...
  // the workspace directory, not to create it again.
  throw new UoaRosterUnavailableError(
    `[uoa] organisation ${externalOrgId} was created but its default workspace could not be resolved`,
  )
}

/**
 * Create a UOA organisation owned by `ownerUoaSub`, and answer the workspace to
 * switch onto.
 *
...
export const createUoaOrganisation = async (
  ...
  deps: UoaRosterDeps = {},
): Promise<UoaProvisionedWorkspace> => {
...
/**
 * Create a further workspace (UOA team) inside an organisation the caller
 * already belongs to.
 *
 ...
 * binds cleanly. Pass one via `withUoaRosterSubjectAssertion(workspace,
 ...
 * deployment.
 *
 * `workspace.externalTeamId` is the caller's CURRENT team, used only to bind
 * the assertion. The new team's id comes back in the response.
 */
export const createUoaWorkspaceTeam = async (
  ...
  if (!externalTeamId) {
    throw new UoaRosterUnavailableError(
      '[uoa] the org API accepted the workspace but returned no id',
    )
  }
  return { externalOrgId: workspace.externalOrgId, externalTeamId }
}
