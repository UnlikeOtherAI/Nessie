...
/**
 * One person to tell. Watchers are people: an agent starts work from a
 * `ticket_changed` trigger instead (`board-watchers.ts`), so a legacy agent
 * row is never a recipient.
 */
export type BoardWatchRecipient = { kind: 'user'; userId: string; boardId: string; taskIds: string[] }
...
export const resolveBoardWatchRecipients = async (
  ...
  for (const event of events) {
    ...
    const watchers = await prisma.boardWatcher.findMany({
      where: { boardId: { in: boardIds }, userId: { not: null } },
      select: { boardId: true, userId: true },
    })
    for (const watcher of watchers) {
      if (!watcher.userId) continue
