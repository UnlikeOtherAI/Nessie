...
export type UoaAutomaticMembershipOperation = { operationId: string; status: 'accepted' | 'completed' | 'already_member' | 'skipped_inactive' | 'failed' }
export interface UoaAutomaticMembershipAdapter {
  attestVerifiedDomain(input: { externalOrgId: string; uoaSub: string; domain: string }): Promise<UoaVerifiedDomainAttestation | null>
  listTeams(input: { externalOrgId: string }): Promise<readonly UoaAutomaticMembershipTeam[]>
  assertRuleAdministrator(input: { externalOrgId: string; externalTeamIds: readonly string[]; externalTeamId?: string; scope: 'organization' | 'team'; uoaSub: string }): Promise<boolean>
...
const key = (): string | null => {
  ...
  return value?.startsWith('uoa_app_') ? value : null
}
...
export const createProductionUoaAutomaticMembershipAdapter = (): UoaAutomaticMembershipAdapter | null => {
  ...
  const operation = (body: Record<string, unknown>): UoaAutomaticMembershipOperation => {
    ...
    if (!operationId || !status || !['accepted', 'completed', 'already_member', 'skipped_inactive', 'failed'].includes(status)) invalid('invalid operation response')
    return { operationId: operationId!, status: status! as UoaAutomaticMembershipOperation['status'] }
  }
  return {
    async attestVerifiedDomain(input) {
      const response = await safeFetch(new URL('/org/automatic-membership/attestations', settings.baseUrl), { method: 'POST', headers: { Accept: 'application/json', Authorization: `Bearer ${appKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ uoaSub: input.uoaSub, org_id: input.externalOrgId, domain: input.domain }), signal: AbortSignal.timeout(10_000) }, { maxRedirects: 0 })
    ...
    async assertRuleAdministrator(input) {
      const scope = input.scope === 'organization' ? 'organisation' : 'team'
      const body = await request(`/org/automatic-membership/organisations/${encodeURIComponent(input.externalOrgId)}/authorizations`, { method: 'POST', body: JSON.stringify({ subject: input.uoaSub, scope, team_id: input.externalTeamId, team_ids: input.externalTeamIds }) })
      const authorization = record(body.authorization)
      return body.allowed === true
        && authorization.subject === input.uoaSub
        && authorization.org_id === input.externalOrgId
        && authorization.scope === scope
        && authorization.team_id === (input.scope === 'team' ? input.externalTeamId ?? null : null)
    },
