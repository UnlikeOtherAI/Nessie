...
const whereFor = (scope: AgentTriggerScope): Prisma.AgentTriggerWhereInput => ({ id: scope.triggerId, agent: { organizationId: scope.organizationId } })
...
export const updateAgentTrigger = async (prisma: PrismaClient, scope: AgentTriggerScope, input: { config?: Record<string, unknown>; description?: string | null; enabled?: boolean; name?: string | null; nextRunAt?: string | null; status?: AgentTriggerStatus; targetChannelId?: string | null; targetThreadId?: string | null }): Promise<AgentTriggerRecord | null> => {
  ...
  const target = targetChanged ? await resolveExecutionTarget(prisma, agentId, { targetChannelId: input.targetChannelId === undefined ? existing.targetChannelId : input.targetChannelId, targetThreadId: input.targetThreadId === undefined ? existing.targetThreadId : input.targetThreadId }) : { channelId: existing.targetChannelId, threadId: existing.targetThreadId }
  if (!target?.channelId || !target.threadId) return null
  ...
  const trigger = Object.hasOwn(configRecord, 'todoTemplateId') && (input.config !== undefined || input.enabled === true)
    ? await prisma.$transaction(async (tx) => { await acquireAgentTodoAgentLock(tx, agentId); return await validateTodoTemplateTriggerConfig(tx, agentId, configRecord) ? write(tx) : null })
    : await write(prisma)
