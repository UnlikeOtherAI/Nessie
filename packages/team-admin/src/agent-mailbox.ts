 ...
 * team-visible agent. Private agents are excluded because the private-agent
...
export const assertMailboxEligible = (agent: MailboxAgent): void => {
  ...
  if (agent.visibility === 'private') {
    throw new AgentMailboxError(
      ...
      + 'mailbox. Publish the agent to the team first.',
    )
  }
    ...
    return await prisma.$transaction(async (tx) => {
      const channel = await tx.channel.create({
        data: {
          ...
          teamId,
          // Team-visible agents only (see assertMailboxEligible), so the
