...
/**
 * Creating a UOA organisation and team against the agreed contract. The
...
test('creating an organisation uses backend mode and returns the default team', async () => {
  await withUoaEnv(async () => {
    const calls: StubCall[] = []
    const team = await createUoaOrganisation(
      { name: 'Acme Ltd', ownerUoaSub: 'uoa-sub-1' },
      deps(calls, [() => json(createdOrg({ id: 'team_general', isDefault: true }))]),
    )

    assert.deepEqual(team, { externalOrgId: 'org_new', externalTeamId: 'team_general' })
...
test('the fallback accepts a lone team even when nothing is flagged default', async () => {
  await withUoaEnv(async () => {
    ...
    assert.equal(team.externalTeamId, 'team_only')
  })
})
...
test('a create answered without an id is a failure, not a half-built team', async () => {
  await withUoaEnv(async () => {
    const calls: StubCall[] = []
    await assert.rejects(
      createUoaOrganisation(
...
test('creating a team uses user mode and keeps the caller organisation', async () => {
  await withUoaEnv(async () => {
    const calls: StubCall[] = []
    const created = await createUoaTeamTeam(
    ...
    // `join_creator` is what puts the caller IN the team they just made.
    ...
    // TeamMember, so without this the person creates a team they cannot
    // open.
