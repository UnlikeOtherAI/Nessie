...
import type { Spy } from './auth-login-team-target-fixture.js'

/**
 * In-memory Prisma fake covering exactly the queries the team-switch
...
export const makePrisma = (spy: Spy, input: SeedInput) => {
   ...
   * `deriveUoaTeamDirectoryFromTeams` reads
  ...
  // provisioned into its target by definition of the shared team.
  ...
  const prisma = {
    // Advisory locks and the team/org bootstrap probes.
    ...
    team: {
      ...
      // `deriveUoaTeamDirectoryFromTeams` narrows to the teams this user
      ...
      findMany: async ({ where, select }: { where?: Row; select?: Row } = {}) => {
        ...
        const matches = teams.filter((team) => {
          ...
          return notNull(where?.externalOrgId, team.externalOrgId)
            && notNull(where?.externalTeamId, team.externalTeamId)
    ...
    channel: {
      ...
      count: async ({ where }: { where?: Row }) => (record('channel.count'),
        channels.filter((channel) => channel.projectId === where?.projectId).length),
    ...
    agent: {
      ...
      // successful login — it is team plumbing, not an identity write.
