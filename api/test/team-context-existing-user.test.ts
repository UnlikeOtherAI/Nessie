 ...
 * resolveUoaTeamContext: `existingUserId` locks and reuses exactly that
...
const makeFake = (seed: {
  ...
  const client = {
      ...
      findUnique: async ({ where }: { where: Row }) => {
        ...
        const found = state.teams.find((team) =>
          ...
          || (where.externalTeamId !== undefined
            && team.externalTeamId === where.externalTeamId)) ?? null
    ...
    projectMember: {
      ...
      updateMany: async ({ where, data }: { where: Row; data: Row }) => {
        record('projectMember.updateMany')
        const rows = state.projectMembers.filter((member) =>
          Object.entries(where).every(([key, value]) => member[key] === value))
        for (const row of rows) Object.assign(row, data)
        return { count: rows.length }
      },
    ...
    teamMember: {
      ...
      updateMany: async ({ where, data }: { where: Row; data: Row }) => {
        record('teamMember.updateMany')
        const rows = state.teamMembers.filter((member) =>
          Object.entries(where).every(([key, value]) => member[key] === value))
        for (const row of rows) Object.assign(row, data)
        return { count: rows.length }
      },
...
const team = (activeTeamId: string): ExternalAuthTeam => ({
...
test('recovery reuses the exact existing principal without any email lookup', async () => {
  ...
  const context = await resolveUoaTeamContext(client, {
    displayName: 'Alice Example',
...
test('a cross-org recovery materializes the TARGET org and claims ITS link', async () => {
  // A team-switch reauthorization can legitimately land in a UOA
...
test('the conditional claim runs BEFORE the target existing-or-create branch', async () => {
  // The fence is the seam between the team advisory lock and any
  ...
  // The team advisory lock and the exact-principal read happened; the
...
test('a refusing claim leaves no shared target rows behind for a NEW team', async () => {
  // Regression for the pre-refactor shape, where resolveTeamTarget ran
...
test('a mismatch between the claim org and the resolved org is impossible by construction', async () => {
  ...
  // hands the claim the exact TARGET org it resolved from the team
