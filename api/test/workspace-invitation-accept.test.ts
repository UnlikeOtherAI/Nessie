...
import { registerWorkspaceInvitationAcceptanceRoute } from '../src/routes/workspace-invitations.js'

const AUTH_SECRET = 'workspace-invite-route-secret'
...
const actorContext: AuthorizedActionContext = {
  ...
  actionContext: { requestId: 'workspace-invite-accept' },
}
...
const makeApp = async (input: {
  ...
  registerWorkspaceInvitationAcceptanceRoute(app, {
    authSecret: AUTH_SECRET,
    getAuthorizationToken: () => token,
    prisma,
    requireActorContext: () => actorContext,
  } as unknown as Parameters<typeof registerWorkspaceInvitationAcceptanceRoute>[1], {
    fetchImpl,
...
const request = {
  ...
  url: '/api/workspace/invitations/invite-1/accept',
}
  ...
  await withUoaEnv(async () => {
    ...
    try {
      ...
      assert.deepEqual(deleted, [{
        where: {
          eventKey: 'workspace-invite:invite-1',
