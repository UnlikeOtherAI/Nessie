...
/**
 * Renaming a UOA-bound workspace is a write to UnlikeOtherAI, not to the local
 ...
 * unable to rename the workspace they were looking at.
 */
  ...
  const prisma = {
    team: {
      findFirst: async () => ({
        ...
        externalWorkspaceId: bound ? externalTeamId : null,
        id: teamId,
      }),
  ...
  registerTeamRoutes(
    app,
    {
      ...
      requireActorContext: () =>
        ({
          actionContext: {
            requestId: 'request-workspace-rename',
...
test('renaming a UOA-bound workspace writes to UnlikeOtherAI first', async () => {
  await withUoaEnv(async () => {
    ...
    // the workspace label are healed, not just the Team.
...
test('a refusal from UnlikeOtherAI leaves the local name untouched', async () => {
  await withUoaEnv(async () => {
    ...
    assert.equal(response.json().error.code, 'WORKSPACE_RENAME_REJECTED')
    assert.equal(calls.length, 1)
...
test('a session with no UOA identity cannot rename a UOA-bound workspace', async () => {
  await withUoaEnv(async () => {
...
test('a local-mode workspace still renames locally with no upstream call', async () => {
  await withUoaEnv(async () => {
