...
import {
  makePrisma,
  type FakeInput,
} from './auth-login-team-target-prisma.js'

/**
 * Shared fixture for the team-switch recovery login route tests: module
 ...
 * auth-login-team-target-prisma.ts; its `model.method` call log lets
...
const dbStub = [
  ...
  '  throw new Error("@nessie/db is stubbed in the team-target tests")',
  '}',
  ...
  '  throw new Error("knowledge-space visibility is not used by the team-target tests")',
  '}',
...
/** Mint the current Nessie bearer Alice holds while switching teams. */
...
/** UOA's exchange response for a subject returning to the expected team. */
...
export const recoveryExchange = (
  ...
  app.inject({
    ...
    payload: JSON.stringify({
      ...
      ...(input.expectedTeam
        ? { expectedTeam: input.expectedTeam }
        : {}),
    }),
  })
