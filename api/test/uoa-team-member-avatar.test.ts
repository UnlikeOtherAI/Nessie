...
import { clearTeamRosterSubjectCache } from '../src/services/uoa-roster-subjects.js'
 ...
 * hash can see, so a subject outside this team's roster must be refused
...
const withUoaEnv = async (run: () => Promise<void>): Promise<void> => {
  ...
  // The roster subject set is cached per team; each test drives its own
  // upstream stub, so none of them may inherit another's roster.
  clearTeamRosterSubjectCache()
  try {
    await run()
  } finally {
...
const actorContextFor = (
  ...
  actionContext: { requestId: 'req-team-member-avatar' },
})
...
test('a roster member’s avatar is relayed to any member of the team', async () => {
  await withUoaEnv(async () => {
    ...
    try {
      const response = await app.inject({
        method: 'GET',
        url: '/api/team/members/usr_grace/avatar',
      })
...
test('a subject outside the roster is a 404 and never reaches the avatar endpoint', async () => {
  await withUoaEnv(async () => {
    ...
    try {
      const response = await app.inject({
        method: 'GET',
        url: '/api/team/members/usr_outsider/avatar',
      })
...
test('the roster read behind the check is asked once per team, not once per row', async () => {
  await withUoaEnv(async () => {
    const calls: StubCall[] = []
    const app = makeApp(
      actorContextFor(['member']),
    ...
    try {
      const responses = await Promise.all(
        ['usr_ada', 'usr_grace'].map((uoaSub) =>
          app.inject({ method: 'GET', url: `/api/team/members/${uoaSub}/avatar` })),
      )
...
test('a team with no UOA team 404s the avatar without reaching UOA', async () => {
  await withUoaEnv(async () => {
    const app = makeApp(
    ...
    try {
      const response = await app.inject({
        method: 'GET',
        url: '/api/team/members/usr_ada/avatar',
      })
...
test('an unreadable roster is a 502, never a silent "not a member"', async () => {
  await withUoaEnv(async () => {
    ...
    try {
      ...
      await app.inject({ method: 'GET', url: '/api/team/members/usr_ada/avatar' })
