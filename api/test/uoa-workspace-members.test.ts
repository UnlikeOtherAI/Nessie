...
import { resolveUoaRosterWorkspace } from '../src/services/uoa-org-roster.js'
...
const teams: TeamRow[] = [
  ...
  // Never provisioned from a UOA workspace — a purely local team.
...
test('resolveUoaRosterWorkspace maps the session team to its UOA org + team', async () => {
  await withUoaEnv(async () => {
    const prisma = makePrisma()

    assert.deepEqual(
      await resolveUoaRosterWorkspace(prisma, { organizationId, teamId }),
    ...
    assert.equal(
      await resolveUoaRosterWorkspace(prisma, { organizationId, teamId: localTeamId }),
      null,
    )
    assert.equal(await resolveUoaRosterWorkspace(prisma, { organizationId, teamId: null }), null)
    assert.equal(
      await resolveUoaRosterWorkspace(prisma, { organizationId: otherOrganizationId, teamId }),
      null,
    )
  })
})

test('GET /api/workspace/members joins the UOA team and organisation reads', async () => {
  await withUoaEnv(async () => {
    ...
    try {
      const response = await app.inject({ method: 'GET', url: '/api/workspace/members' })
  ...
  await withUoaEnv(async () => {
    ...
    try {
      const response = await app.inject({
        method: 'DELETE',
        url: '/api/workspace/members/usr_ghost',
      })
      assert.equal(response.statusCode, 404)
      assert.equal(response.json().error.code, 'WORKSPACE_MEMBERS_REJECTED')

      const rejected = await app.inject({
        method: 'PUT',
        url: '/api/workspace/members/usr_ada/role',
        payload: { role: 'admin' },
      })
      assert.equal(rejected.statusCode, 400)
      assert.equal(rejected.json().error.code, 'WORKSPACE_MEMBERS_REJECTED')
    } finally {
      await app.close()
    }
  })
...
test('a team with no UOA workspace 404s and never reaches UOA', async () => {
  await withUoaEnv(async () => {
    const app = await makeApp(
...
test('the roster read is open to any member of the workspace', async () => {
  await withUoaEnv(async () => {
    const calls: StubCall[] = []
    const app = await makeApp(
  ...
  for (const role of ['member', 'viewer']) {
    await withUoaEnv(async () => {
      ...
      const requests = [
        ...
        { method: 'DELETE' as const, url: '/api/workspace/members/usr_ada' },
        { method: 'POST' as const, url: '/api/workspace/members/usr_ada/deactivate' },
        { method: 'POST' as const, url: '/api/workspace/members/usr_ada/reactivate' },
      ]
  ...
  for (const role of ['owner', 'admin']) {
    await withUoaEnv(async () => {
      ...
      const expected: {
        ...
        {
          request: { method: 'GET', url: '/api/workspace/invitations' },
        ...
        {
          request: { method: 'POST', url: '/api/workspace/invitations/inv_1/resend' },
          upstream: {
            method: 'POST',
        ...
        {
          request: { method: 'POST', url: '/api/workspace/invitations/inv_1/approve' },
          upstream: { method: 'POST', url: `${base}/invitations/inv_1/approve${query}` },
        },
        {
          request: { method: 'POST', url: '/api/workspace/invitations/inv_1/deny' },
        ...
        {
          request: { method: 'DELETE', url: '/api/workspace/members/usr_ada' },
          upstream: {
            method: 'DELETE',
            url: `${base}/teams/${externalTeamId}/members/usr_ada${query}`,
          },
        ...
        {
          request: { method: 'POST', url: '/api/workspace/members/usr_ada/deactivate' },
          upstream: { method: 'POST', url: `${base}/members/usr_ada/deactivate${query}` },
        },
        {
          request: { method: 'POST', url: '/api/workspace/members/usr_ada/reactivate' },
...
test('workspace member routes are inert when UOA is not configured', async () => {
