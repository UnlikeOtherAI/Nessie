...
/**
 * Success-and-discriminant coverage for the account-bound team-switch
...
test('a wrong organization target is refused BEFORE the billing confirm', async () => {
  ...
  const response = await recoveryExchange(app, {
    bearer: aliceBearer(),
    expectedTeam: { organizationId: 'uoa-org-b', teamId: TEAM },
  })

  assert.equal(response.json().error.code, 'TEAM_TARGET_MISMATCH')
  assertUntouched(response, spy, upstream)
})
...
test('a wrong team within the same organization is refused BEFORE the billing confirm', async () => {
  ...
  const response = await recoveryExchange(app, {
    bearer: aliceBearer(),
    expectedTeam: { organizationId: ORG, teamId: 'uoa-team-b' },
  })
...
test('a cross-org reauthorization lands in the freshly materialized TARGET org', async () => {
  ...
  // (a team she is entitled to but has never entered locally). Under the
  ...
  const response = await recoveryExchange(app, {
    ...
    expectedTeam: { organizationId: 'uoa-org-b', teamId: 'uoa-team-b' },
  })
