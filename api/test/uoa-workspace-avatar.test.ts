...
import { resolveUoaWorkspace } from '../src/services/uoa-avatar.js'
...
test('resolveUoaWorkspace maps the session team to its UOA workspace', async () => {
  ...
  assert.equal(
    await resolveUoaWorkspace(prisma, { organizationId, teamId: localTeamId }),
    null,
  )
  assert.equal(await resolveUoaWorkspace(prisma, { organizationId, teamId: null }), null)
  assert.equal(
    await resolveUoaWorkspace(prisma, {
...
test('GET /api/workspace/avatar relays the company image for any member', async () => {
...
test('GET /api/workspace/avatar 404s a team with no UOA workspace', async () => {
  await withUoaEnv(async () => {
...
test('PUT /api/workspace/avatar relays an owner or admin upload as multipart', async () => {
...
test('PUT and DELETE /api/workspace/avatar reject non-admin members', async () => {
...
test('PUT /api/workspace/avatar rejects a non-raster upload before relaying', async () => {
  await withUoaEnv(async () => {
...
test('PUT /api/workspace/avatar rejects an image over the 1 MiB ceiling', async () => {
  await withUoaEnv(async () => {
    const app = await makeApp(actorContextFor(['owner']))
    const originalFetch = globalThis.fetch
    globalThis.fetch = (async () => {
...
test('PUT /api/workspace/avatar surfaces a UOA rejection as a 400', async () => {
  await withUoaEnv(async () => {
    const app = await makeApp(actorContextFor(['owner']))
    const originalFetch = globalThis.fetch
    globalThis.fetch = (async () =>
...
test('DELETE /api/workspace/avatar clears the image for an owner', async () => {
  await withUoaEnv(async () => {
    const app = await makeApp(actorContextFor(['owner']))
    const calls: StubCall[] = []
    const originalFetch = globalThis.fetch
...
test('GET /api/workspace/avatar reports an upstream failure as 502', async () => {
  await withUoaEnv(async () => {
    const app = await makeApp(actorContextFor(['member']))
    const originalFetch = globalThis.fetch
    globalThis.fetch = (async () =>
...
test('workspace avatar routes are inert when UOA is not configured', async () => {
  ...
  try {
    const read = await app.inject({ method: 'GET', url: '/api/workspace/avatar' })
