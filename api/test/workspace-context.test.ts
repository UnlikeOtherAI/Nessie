...
// resolveUoaWorkspaceContext performs. A UOA login (workspace claim carrying
...
// no-workspace fallback still uses. Stateful so "same workspace → same team" /
...
const makeFake = (seed?: { organizationId?: string; withDefaultTeam?: boolean }) => {
  ...
  const client = {
    ...
    // row so a workspace switch can sync the target org's links.
    ...
    // The account-link upsert of `syncUoaProductAccountLinks` (workspace
...
test('auto-provisions the per-UOA-org Organization and its workspace team', async () => {
  ...
  // Placeholder name until the workspace directory supplies UOA's orgName.
...
test('two UOA organisations resolve to two distinct local Organizations', async () => {
  ...
  const orgA: ExternalAuthWorkspace = {
    activeOrgId: 'uoa-org-a',
    activeTeamId: 'ws-a',
    teamIds: ['ws-a'],
    teamRoles: {},
  }
  const orgB: ExternalAuthWorkspace = {
  ...
  const a = await resolveUoaWorkspaceContext(client, identityFor('a@x.com', orgA, 'sub-1'))
  const b = await resolveUoaWorkspaceContext(client, identityFor('a@x.com', orgB, 'sub-1'))
  ...
  const again = await resolveUoaWorkspaceContext(client, identityFor('a@x.com', orgA, 'sub-1'))
...
test('no workspace selection scopes to the shared org, never a UOA org (legacy behaviour)', async () => {
  const orgId = randomUUID()
  const { client, defaultTeamId } = makeFake({ organizationId: orgId, withDefaultTeam: true })

  // A UOA login materialises its per-org workspace; a later login with no
  // workspace claim at all (generic OIDC shape) stays in the legacy shared
...
test('a cross-org workspace switch materializes the TARGET organization and its links', async () => {
  const { client, state } = makeFake({})
  const source = await resolveUoaWorkspaceContext(
    client,
    identityFor('mover@x.com', workspace('ws-source', 'member'), 'uoa-sub-mover'),
  )
  assert.ok(source)
  const target = { organizationId: 'uoa-org-2', teamId: 'ws-elsewhere' }
  const targetWorkspace: ExternalAuthWorkspace = {
  ...
  await materializeUoaWorkspaceSwitch(client, {
    identity: {
      displayName: 'Mover',
      email: 'mover@x.com',
  ...
  const targetTeam = state.teams.find((t) => t.externalWorkspaceId === target.teamId)
  assert.ok(targetTeam)
  assert.equal(
...
test('with no org_role claim a LATER joiner of an existing org defaults to member', async () => {
  ...
  await resolveUoaWorkspaceContext(client, identityFor('first@x.com', workspace('ws-eng')))
  const ctx = await resolveUoaWorkspaceContext(client, identityFor('later@x.com', workspace('ws-eng')))
...
test('a verified org_role claim outranks the first-org-materializer rule', async () => {
  const { client, state } = makeFake({})
  const asMember: ExternalAuthWorkspace = {
    ...workspace('ws-eng', 'member'),
    orgRole: 'member',
  }

  const ctx = await resolveUoaWorkspaceContext(client, identityFor('a@x.com', asMember))
...
test('a login with no UOA claims leaves an existing role untouched', async () => {
  ...
  assert.ok(first)
  // A generic (non-UOA) OIDC login: no workspace claim at all. It scopes to
