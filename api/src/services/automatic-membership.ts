...
export const verifyAutomaticMembershipClaim = async (
  ...
  if (!matched) {
    ...
    for (const dependent of fences) {
      await setUoaFence({ externalOrgId: organization?.externalOrgId ?? null, ruleId: dependent.id, generation: dependent.generation, fenceToken: dependent.token, active: false })
    }
    await prisma.$transaction(async (tx) => {
      for (const dependent of fences) {
        const changed = await tx.automaticMembershipRule.updateMany({
          where: { id: dependent.id, generation: dependent.generation - 1, state: 'active' },
          data: { state: 'suspended', generation: dependent.generation, uoaFenceToken: dependent.token, suspensionReason: 'DNS verification no longer passed.' },
        })
        if (changed.count !== 1) throw new AutomaticMembershipError('RULE_CHANGED', 'A rule changed while DNS verification failed. Refresh and try again.', 409)
      }
      await tx.automaticMembershipDomainClaim.update({ where: { id: rule.claimId }, data: { state: 'suspended', lastDnsCheckAt: now, lastDnsFailure: 'TXT record was missing or did not match' } })
      await audit(tx, context, 'automatic_membership.suspended', rule.id, { reason: 'dns_mismatch' })
      await audit(tx, context, 'automatic_membership.dns_checked', rule.id, { matched: false })
    })
    return { verified: false, rule: presentRule({ ...rule, state: 'suspended', claim: { ...rule.claim, state: 'suspended', lastDnsCheckAt: now } }) }
  }
  const updated = await prisma.$transaction(async (tx) => {
    const claim = await tx.automaticMembershipDomainClaim.update({ where: { id: rule.claimId }, data: { state: 'verified', verifiedAt: now, verificationExpiresAt: expiresAt, lastDnsCheckAt: now, lastDnsFailure: null } })
    await audit(tx, context, 'automatic_membership.verified', rule.id, { matched: true })
    return claim
  })
