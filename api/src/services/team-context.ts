...
import {
  ...
  readOrgRole,
} from './team-principal.js'
import {
  lockExternalTeam,
  materializeTeamTargetInTransaction,
  resolveDefaultTarget,
  resolveTeamTarget,
  TeamExternalBindingConflictError,
} from './team-target.js'
import {
...
// Kept importable from here: uoa-team-switch.ts catches this exact class
// to map binding conflicts to its switch refusal.
export { TeamExternalBindingConflictError }
...
// selected UOA team inside it. `orgRole` is the org membership role the
// access token is scoped to (RBAC is org-role driven). Logins with no
// team claim (generic OIDC, legacy) keep the shared-org behaviour.
...
export type TeamIdentityInput = {
   ...
   * team claim's org id (a cross-org switch is legitimate and lands in
   * the target org — which may be materialized by this very recovery); this
   * claim is only the fallback scope for a recovery whose team claim
   ...
   * team plus its memberships for this id. Used by the team-switch
   ...
   * external-org and external-team advisory locks, then this claim
  ...
  // principal resolution then keys on it (team-principal.ts); generic
...
//      exact external-team advisory lock (target serialization),
...
// The target org comes from the verified team claim's external org id —
...
const resolveRecoveryContext = async (
...
): Promise<TeamContext | null> => prisma.$transaction(async (tx) => {
  const externalOrgId = input.externalTeamId
    ? resolveExternalTeamSelection(input.team).organizationId
    : null
  if (input.externalTeamId) {
    if (externalOrgId) {
      await lockExternalOrganization(tx, externalOrgId)
    }
    await lockExternalTeam(tx, externalOrgId, input.externalTeamId)
  }
  ...
  const target = input.externalTeamId
    ? await materializeTeamTargetInTransaction(
        tx,
        organizationId,
  ...
  const firstOrgMember = externalOrgId
    ...
    : false
  // Recovery is a login-equivalent for the target team: the memberships
  ...
  await ensureTeamMemberships(tx, {
    userId: existing.id,
    organizationId,
...
const initializeSharedOrganization = async (
  prisma: PrismaClient,
  input: TeamIdentityInput & { email: string; existingUserId?: undefined },
): Promise<{
  sharedOrg: { id: string }
  user: { id: string } | null
  ...
  const seeded = await seedBootstrapRecordsInTransaction(transaction, {
    ...
    // provider asserted none. `ensureTeamPrincipal` re-syncs the mirror
 ...
 * Organization, team → team). Returns the org/project/team the session is
 * scoped to.
 *
 * - `existingUserId` set (account-bound recovery) → ONE transaction:
 *   external-org + external-team locks, target-organization
 ...
 * - Team selection with an external org id → the per-UOA-org Organization
 ...
 *   team project+team materialized INSIDE it. First entry into a
 ...
 * - No team selection → the shared/default org and the user's
 *   existing/default team (generic OIDC and legacy logins, byte-for-byte).
 * - Known team, team exists → join it.
 * - Known team, no team yet → auto-provision it; the first person owns it
 *   unless UOA sent a role for that team, which wins.
 *
 ...
 */
export const resolveUoaTeamContext = async (
  prisma: PrismaClient,
  input: TeamIdentityInput,
): Promise<TeamContext | null> => {
  // Recovery: the target org comes from the verified team claim (falling
  ...
  if (input.existingUserId) {
    ...
    return resolveRecoveryContext(prisma, {
      ...
      team: input.team,
      externalTeamId:
        resolveExternalTeamSelection(input.team).teamId ?? undefined,
    })
  }

  const email = input.email.trim().toLowerCase()
  const selection = resolveExternalTeamSelection(input.team)
  const externalTeamId = selection.teamId ?? undefined

  // A team selection carrying an external org id is structurally a UOA
  // login: the UOA organisation maps 1:1 to a local Organization. Everything
  // else (generic OIDC, legacy no-team logins) keeps the shared-org path
  // below byte-for-byte.
  if (externalTeamId && selection.organizationId) {
    const externalOrgId = selection.organizationId
    const resolved = await prisma.$transaction(async (tx) => {
      // Lock order: external-org, then external-team — the same order
      ...
      const target = await materializeTeamTargetInTransaction(
        tx,
        organization.id,
        externalTeamId,
        input.team,
    ...
    const principal = await ensureTeamPrincipal(prisma, {
      avatarUrl: input.avatarUrl,
  ...
  // 2. Resolve the target environment (team team, or default team).
  const target = externalTeamId
    ? await resolveTeamTarget(prisma, organizationId, externalTeamId, input.team)
