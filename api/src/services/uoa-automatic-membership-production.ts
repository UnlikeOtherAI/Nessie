...
export const createProductionUoaAutomaticMembershipAdapter = (): UoaAutomaticMembershipAdapter | null => {
  ...
  const operation = (body: Record<string, unknown>): UoaAutomaticMembershipOperation => {
    ...
    return fail('invalid operation response')
  }
  return {
    async attestVerifiedDomain(input): Promise<UoaVerifiedDomainAttestation | null> {
      const body = await request('/org/automatic-membership/attestations', {
        method: 'POST',
        body: JSON.stringify(input),
      })
      const domain = string(body.domain)
      const uoaSub = string(body.subject)
      const assertedAt = string(body.asserted_at)
      const expiresAt = string(body.expires_at)
      if (
        !domain || !uoaSub || !assertedAt || !expiresAt
        || uoaSub !== input.uoaSub || domain !== input.domain
      ) return null
      const asserted = new Date(assertedAt); const expires = new Date(expiresAt)
      if (Number.isNaN(asserted.getTime()) || Number.isNaN(expires.getTime()) || expires <= new Date()) {
        return null
      }
      return { uoaSub, domain, assertedAt: asserted, expiresAt: expires }
    },
    ...
    async listVerifiedDomainSubjects(input): Promise<UoaDomainSnapshotPage> {
      ...
      const body = await request(
        `/org/automatic-membership/organisations/${encodeURIComponent(input.externalOrgId)}/subjects?${query}`,
      )
