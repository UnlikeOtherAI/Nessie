...
import { UoaSubjectConflictError } from './team-principal.js'
...
const mapDirectAccessFailure = (error: UoaBillingError): never => {
  if (
    ...
    throw new UoaTeamSwitchError(
      'TEAM_NOT_AVAILABLE',
      'UnlikeOtherAI did not confirm Nessie access in the target team.',
      true,
    )
  }
  if (
    ...
    throw new UoaTeamSwitchError(
      ...
      'UnlikeOtherAI requires an interactive sign-in for this team.',
      true,
    )
  }
...
export const confirmUoaTeamSwitchAccess = async (
  input: {
...
/**
 * Idempotently materialize the local org/project/team for the team UOA
 ...
 * `materializeUoaTeamSwitch`, which first pins the response to the
 * requested target) and an ordinary refresh whose successor carries a drifted
 * team — adopting that drift has to land somewhere real or fail closed,
...
export const materializeUoaTeam = async (
  ...
  let context: Awaited<ReturnType<typeof resolveUoaTeamContext>>
  try {
    ...
    if (error instanceof UoaUnrecognizedRoleError) {
      // The target team claims a role this deployment cannot express as a
    ...
    if (error instanceof TeamExternalBindingConflictError) {
    ...
    throw new UoaSessionRefreshError(
      'The target Nessie team is temporarily unavailable.',
      false,
    )
  }
...
/**
 * Materialize an explicit switch: the response must prove the exact team
...
export const materializeUoaTeamSwitch = async (
  ...
  if (
    ...
    throw new UoaRefreshBindingError(
      'UnlikeOtherAI returned a different team while materializing the switch.',
    )
  }
