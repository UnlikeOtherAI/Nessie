...
import { renameCachedUoaWorkspace } from '../services/uoa-directory-cache.js'
import {
  renameUoaWorkspace,
...
import { mirrorExternalWorkspaceName } from '../services/workspace-target.js'
import { requireLocalMembershipManagement } from './membership-mode-gate.js'
import type { RouteDeps } from './types.js'

/**
 * Renaming a workspace whose name UOA owns is a write to UOA, not to the local
 * mirror.
 *
 ...
 * mirror that `syncExternalWorkspaceNames` heals from UOA's verified directory.
 ...
 * UOA's own admin to rename their workspace. It now relays instead: UOA stays
 ...
 * nothing locally. A local install with no IdP (`externalWorkspaceId` null)
 ...
 * upstream call this file makes — renaming a UOA-bound workspace. Production
...
export const registerTeamRoutes = (
  ...
  const UOA_SESSION_MESSAGE =
    'This workspace is managed by UnlikeOtherAI. Sign in with UnlikeOtherAI and select this '
    + 'workspace to rename it.'

  /**
   * Relay a workspace rename to UnlikeOtherAI and answer with the name UOA
   ...
   * additionally requires the asserted workspace to equal the one in the route,
   * so this can only ever rename the workspace the caller is standing in. That
  ...
  const renameOnUnlikeOtherAI = async (
    ...
    try {
      return await renameUoaWorkspace(
        workspace,
        input.name,
        withUoaRosterSubjectAssertion(
          workspace,
      ...
      if (error instanceof UoaRosterUnavailableError) {
        request.log.warn({ err: error }, 'uoa workspace rename relay failed')
        sendApiError(
          reply,
          502,
  ...
  app.patch('/api/teams/:teamId', async (request, reply) => {
    ...
    // local-mode workspace resolves to the requested name with no upstream call.
    const stored = team.externalWorkspaceId
    ...
    if (team.externalWorkspaceId) {
      // Both rows carry the workspace label, so heal them exactly as the
      // directory sync would rather than leaving the Project on the old name.
      await mirrorExternalWorkspaceName(prisma, team.externalWorkspaceId, stored)
      ...
      // rotation — the workspace a person renamed while looking at it.
      renameCachedUoaWorkspace(
