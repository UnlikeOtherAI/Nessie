    ...
    if (body.providerId && body.providerId !== LOCAL_AUTH_PROVIDER_ID) {
      ...
      try {
        ...
        if (uoaSession && !recoveryClaims) {
          ...
          const hasNewAutomaticTarget = automaticTarget
            && (current.organizationId !== automaticTarget.externalOrgId
              || current.teamId !== automaticTarget.externalTeamId)
          if (hasNewAutomaticTarget) {
