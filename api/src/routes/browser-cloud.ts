...
import { completeBrowserLoginHandover } from '../services/browser-login-handover.js'
import {
  AgentCardResponseError,
  respondToAgentCard,
} from '../services/agent-card-response.js'
import { findBrowserLoginCardForViewer } from '../services/browser-login-card-selection.js'
...
/** A signed-in throwaway browser belongs only to the person who opened it. */
export const viewerMaySeeCloudBrowserSession = (input: {
  ...
  if (input.agentBrowserId === null) {
    return (
      (!input.authenticated && input.controlledByUserId === null)
      || input.requestedByUserId === input.viewerId
    )
  }
  if (!input.authenticated) return true
  return input.durableBrowserAllowed
}
...
const agentHasBrowserOpenGrant = async (
  prisma: RouteDeps['prisma'],
...
const loadViewableSession = async (
  ...
  if (session.authenticated && session.agentBrowserId) {
    ...
    if (!viewerMaySeeCloudBrowserSession({
      ...
      durableBrowserAllowed: allowed,
      requestedByUserId: session.requestedByUserId,
      viewerId: input.actorContext.actor.actorId,
    })) return null
  } else if (!viewerMaySeeCloudBrowserSession({
    agentBrowserId: session.agentBrowserId,
    authenticated: session.authenticated,
    controlledByUserId: session.controlledByUserId,
...
export const registerBrowserCloudRoutes = (
  ...
  app.get('/api/threads/:threadId/browser-sessions', async (request, reply) => {
    ...
    const sessions = await prisma.cloudBrowserSession.findMany({
      where: {
        ...
        OR: [
          { agentBrowserId: { not: null }, authenticated: false },
          {
            agentBrowserId: null,
            authenticated: false,
      ...
      select: {
        ...
        endedAt: true,
        controlledByUserId: true,
        agent: { select: { name: true, toolPolicy: true } },
      },
    ...
    return createApiResponse(
      CloudBrowserSessionListSchema.parse({
        sessions: sessions
          .filter((row) => isExplicitToolGranted(row.agent.toolPolicy, BROWSER_OPEN_TOOL_ID))
          .map((row) => ({
          id: row.id,
          agentId: row.agentId,
  ...
  app.delete("/api/browser-sessions/:sessionId", async (request, reply) => {
    ...
    if (session.agentBrowserId) {
      const card = await findBrowserLoginCardForViewer(prisma, {
        agentBrowserId: session.agentBrowserId,
        organizationId: actorContext.tenant.organizationId,
        threadId: session.threadId,
        userId: actorContext.actor.actorId,
    ...
    if (session.controlledByUserId !== actorContext.actor.actorId)
      return reply.code(204).send();
    // Capture while this user still owns the session, then release and wake the
    // ordinary handover path.
    ...
    const handover =
      session.agentBrowserId === null
        ? null
        : await completeBrowserLoginHandover(prisma, {
            agentBrowserId: session.agentBrowserId,
            organizationId: actorContext.tenant.organizationId,
            requestedByUserId:
        ...
        if (!handover)
          await prisma.agentBrowser.update({
            data: {
              ...
              handedBackByUserId: actorContext.actor.actorId,
            },
            where: { id: browser.id },
          });
