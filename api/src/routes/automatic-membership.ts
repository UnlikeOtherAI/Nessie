...
const requireScopeAdministrator = async (
  ...
  reply: FastifyReply,
  scope: Scope,
): Promise<{ context: NonNullable<ReturnType<RouteDeps['requireActorContext']>>; teamId?: string; externalOrgId: string } | null> => {
  ...
  if (scope === 'organization') {
    ...
    if (access.status === 'allowed') return { context, externalOrgId: organization.externalOrgId }
  ...
  return { context, teamId, externalOrgId: organization.externalOrgId }
}

const requireRuleInScope = async (deps: RouteDeps, organizationId: string, ruleId: string, scope: Scope, teamId?: string): Promise<boolean> => {
  if (scope === 'team' && !teamId) return false
  return Boolean(await deps.prisma.automaticMembershipRule.findFirst({
    where: { id: ruleId, organizationId, scope, ...(scope === 'team' ? { targets: { some: { teamId: teamId! } } } : {}) }, select: { id: true },
  }))
}
...
export const registerAutomaticMembershipRoutes = (app: FastifyInstance, deps: RouteDeps): void => {
  ...
  for (const scope of ['organization', 'team'] as const) {
      ...
      app.get(`${base(scope)}/teams`, async (request, reply) => {
        ...
        try {
          return createApiResponse({
            teams: await listAutomaticMembershipTargetTeams(deps.prisma, access.externalOrgId),
          })
    ...
    app.post(base(scope), async (request, reply) => {
      ...
      try {
        return createApiResponse(await createAutomaticMembershipRule(deps.prisma, access.context, scope, body, { authSecret: deps.authSecret ?? '', teamId: access.teamId, externalOrgId: access.externalOrgId }))
      } catch (error) { if (sendServiceError(reply, error)) return reply; throw error }
    })
    ...
    app.post<{ Params: { ruleId: string } }>(`${base(scope)}/:ruleId/suspend`, async (request, reply) => {
      const access = await requireScopeAdministrator(deps, request, reply, scope)
      if (!access) return reply
      if (!(await requireRuleInScope(deps, access.context.tenant.organizationId, request.params.ruleId, scope, access.teamId))) return sendApiError(reply, 404, 'RULE_NOT_FOUND', 'Automatic login rule not found.')
  ...
  app.post<{ Params: { claimId: string } }>('/api/organization/automatic-membership/claims/:claimId/release', async (request, reply) => {
    ...
    if (!access) return reply
    try { await releaseAutomaticMembershipClaim(deps.prisma, access.context, request.params.claimId); return createApiResponse({ ok: true }) } catch (error) { if (sendServiceError(reply, error)) return reply; throw error }
  })
}
