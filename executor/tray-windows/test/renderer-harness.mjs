...
try {
  for (const host of ['windows', 'macos']) {
    ...
    await page.getByRole('button', { name: 'Save interactive program' }).click()
    const terminal = await page.evaluate(() => window.calls.filter((call) =>
      call.command === 'executor_configure' && call.args.configurationInput.terminalProgram).at(-1))
    assert.equal(terminal.args.executorId, 'team-1')
    assert.deepEqual(terminal.args.configurationInput.terminalProgram.command, ['node', '/installed/gemini.js', '--debug'])
