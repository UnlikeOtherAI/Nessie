...
//! into the release, which is what [`super::decide_windows_release_signature`]
//! does with the facts this module collects.
...
/// person launching an app offline must not lose executor controls to a CRL
/// fetch that cannot complete. The state handle is always closed.
pub(super) fn collect_signature_facts(path: &std::path::Path) -> Option<WindowsSignatureFacts> {
