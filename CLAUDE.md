...
second local copy of identity or org structure may exist. **Creating** one runs against UOA's org API in-app, never a second login, local rows still born only in `materializeUoaWorkspace` ([creating](docs/plans/2026-09-02-in-app-organisation-creation.md)).
Full rule: `AGENTS.md` → "UOA owns the org structure"; model and migration:
