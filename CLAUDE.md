...
## Navigation — read `docs/navigation/overview.md` first

Screens, overlays, Back, and the motion between them are one framework:
[docs/navigation/overview.md](docs/navigation/overview.md). Do not add a second way.
...
- **One tab bar, everywhere.** Every single-select strip in the admin — detail
  ...
  [docs/navigation/overview.md](docs/navigation/overview.md).
...
blueprints in `@nessie/workspace-admin`, instantiated by `ensureGlobalAgent` as one
`systemManaged` row per organisation keyed by `Agent.systemSlug`, reachable through a
per-user private home DM (`gagent:{slug}:{orgId}:{userId}`,
`systemChannelType='system_agent'`, one member and one binding, both database facts).
Invariants — the CHECKs, the ensure/policy-merge shape, the binding, trigger and
run-placement refusals, the un-gated list arm, the delegation predicate with its one-arm
identity-tool gate, and the handoff bounds: `AGENTS.md` → "A global agent is a blueprint
in code". Mechanics — toolset and shared reads, the generated capability catalogue,
`agent_handoff`'s delivery, the sidebar's second face, the address book:
...
- Bootstrap runs beside the PA's at login and user provisioning but **best-effort**
  (`attemptGlobalAgentsBootstrap`) — a global agent must never lock anyone out.
  Model resolution is blueprint pin → `NESSIE_DESIGNER_MODEL` → organisation
  default on **both** faces (`resolveGlobalAgentModel`).
- **Designing an agent is the Designer's alone.** `agent_create`, `agent_read`,
  ...
  `identityDelegatedOnly`, removing the PA's own arm from the
  `personalAssistantOnly` gate: the PA keeps the operational verbs on existing
  agents and hands over with `agent_handoff`, those tools omitted from its schema
  array rather than offered and denied.
- **Two faces, one brain.** The Designer page's sidebar keeps its own transport
  (in-process SSE driving the open form, which the DM cannot do) but renders the
  blueprint's persona and the same generated catalogue, reads this organisation's
  tools server-side, searches through Ledger, and wears its name and portrait.
- **A Nessie-managed agent has a read-only configuration view.**
  ...
  entitlement; the admin renders `SystemAgentConfigPanel` in place of the tabs.
  `isAgentAccessibleToActor` is deliberately NOT widened — status, activity,
  messages and children stay closed, a global agent's activity spanning every
  member's private DM.

**Direct messages lists conversations; the address book lists what you can talk to.**
Every DM channel is provisioned before anybody speaks — a person's DM, a private agent's
home, a global agent's home the moment the account exists — so listing provisioned
channels made that section a roster of the workspace with the Agent Designer pinned in
it from day one. A row appears once its channel carries a message, plus the channel the
viewer is standing in, so opening a fresh conversation never pulls its own row out from
under them (`admin/src/layouts/admin-shell/sidebar-dm-lists.ts`); starring is
unaffected, resolving through the full people directory, because starring somebody *is*
adding them. "New message" answers the opposite question, so a DM-homed system agent is
in that address book for everybody — never *bound* into the new channel but **resolved**
to that person's existing home DM, member-level because opening your own home DM is not
placement ([docs/global-agents.md](docs/global-agents.md) → "Addressable, not bound").
The doorways stay named: **Create → Message** (and the section's `+`) reaches a person,
**Create → Agent** opens the Designer's **chat**, the form staying for field edits — the
create menu's last row, and on the phone sheet the last row *above* its morphing Message
button (`mobile/src/lib/native-creation-menu.ts`). One `openAgentDesignerChat` serves
