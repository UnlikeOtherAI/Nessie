...
- **DeepWater as an agent tool**: enabling DeepWater for a team (owner-only
  ...
  **Default OFF, explicit per-agent grant required:** the
  ...
  `LEDGER_PROXY_TOKEN` as Nessie's dedicated, product-bound Ledger app API key;
  personal DeepWater overrides are forbidden. Signed caller identity is a
