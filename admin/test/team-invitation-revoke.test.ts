...
const { TeamMembersSection } = await import('../src/pages/settings/TeamMembersSection.js')
...
const mount = async (
  ...
  const record = (method: string) => async (path: string) => {
    ...
    if (path === '/api/team/members' && options.memberError) throw options.memberError
    if (path === '/api/team/members') return { members: [] }
    if (path === '/api/team/invitations') return invitations
    return { ok: true }
  }
  const apiClient = {
  ...
  if (!options.memberError) queryClient.setQueryData(teamKeys.members, { members: [] })
  queryClient.setQueryData(teamKeys.invitations, invitations)
    ...
    root.render(
      h(
        ...
        h(
          ApiClientProvider,
          { client: apiClient },
          h(TeamMembersSection, { canManage, onReconnect: options.onReconnect }),
        ),
      ),
    )
  })
...
test('an owner or admin can revoke a sent invitation from the pending list', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      [{ method: 'POST', path: '/api/team/invitations/inv_1/revoke' }],
    )
    ...
    assert.ok(
      harness.calls.some(
        (call) => call.method === 'GET' && call.path === '/api/team/invitations',
      ),
    )
    assert.ok(
      harness.calls.some(
        (call) => call.method === 'GET' && call.path === '/api/team/members',
      ),
    )
  } finally {
...
test('a member who cannot manage the team is offered no revoke', async () => {
...
test('an unlinked UOA team offers a reconnection instead of a false outage', async () => {
  let reconnects = 0
  const harness = await mount(true, {
  ...
  try {
    ...
    assert.equal(
      harness.calls.some((call) => call.path === '/api/team/invitations'),
      false,
    )

    await harness.click(harness.buttons('Reconnect team')[0])
...
test('a rejected active team offers reconnection from the live roster error', async () => {
  const harness = await mount(true, {
    ...
    memberError: Object.assign(
      ...
      { code: 'TEAM_MEMBERS_REJECTED', status: 404 },
    ),
    onReconnect: async () => {},
  })
...
test('a UOA directory outage remains distinct from an unlinked team', async () => {
  const harness = await mount(false, {
    memberError: new ApiClientError(
