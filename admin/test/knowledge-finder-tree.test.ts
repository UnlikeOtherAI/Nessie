...
test('Knowledge Finder exposes Tree, Columns and List views', () => {
  ...
  assert.match(documents, /view === 'tree' && !single && !virtualColumnKey && levels\.length > 0/)
  assert.match(documents, /rowsIn=\{rowsIn\}/)
  assert.match(tree, /rowsIn/)
  assert.doesNotMatch(tree, /onOpenVirtual|onOpenRoot|Projects|Agents|Spaces/)
  assert.doesNotMatch(documents, /onSelectSpace|onSelectVirtual/)
