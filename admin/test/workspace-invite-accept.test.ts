...
const { WorkspaceSwitcher } = await import('../src/layouts/admin-shell/WorkspaceSwitcher.js')
...
test('the workspace switcher accepts a pending invitation and enters that workspace', async () => {
  ...
  globalThis.fetch = async (input, init) => {
    ...
    if (path === '/api/workspace/invitations/invite-1/accept') {
      return jsonResponse({ ok: true, organizationId: 'uoa-org-2', teamId: 'uoa-team-2' })
    }
    if (path === '/api/auth/uoa/workspace') {
  ...
  try {
    ...
    assert.deepEqual(
      calls.find((call) => call.path.endsWith('/invite-1/accept')),
      {
        ...
        method: 'POST',
        path: '/api/workspace/invitations/invite-1/accept',
      },
    )
    assert.deepEqual(
      calls.find((call) => call.path === '/api/auth/uoa/workspace'),
      {
        body: { organizationId: 'uoa-org-2', teamId: 'uoa-team-2' },
        method: 'POST',
        path: '/api/auth/uoa/workspace',
      },
    )
