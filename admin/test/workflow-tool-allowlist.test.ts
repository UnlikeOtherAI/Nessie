...
// (packages/runtime) and shared team validation consumes it; the canvas cannot
...
test('shared team validation uses the runtime list (no hand-maintained copy)', () => {
  const source = readFileSync(
    new URL('../../packages/team-admin/src/workflow-template-validation.ts', import.meta.url),
    'utf8',
  )
