...
test('project emojis use no avatar background and unset pictures fall back to the folder glyph', () => {
  ...
  // An emoji is the fallback, never drawn over a picture the project has set.
  assert.match(projectAvatarSource, /avatarEmoji\s*$/m)
  assert.match(projectAvatarSource, /kind: 'glyph', glyph: avatarEmoji/)
  assert.match(projectAvatarSource, /M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v8/)
  // The tile itself is the shared one, so a project matches a person beside it.
  assert.match(projectAvatarSource, /<IdentityTile/)
})
