...
import { ScopedSettingGate } from '../src/components/features/settings/ScopedSettingGate.js'
import { SETTING_KEYS, type ResolvedSetting } from '../src/facades/settings/hooks.js'
...
test('a level below a lock cannot edit, still sees what is in force, and is told which level decided', () => {
  ...
  // Naming the level is `ScopedSettingGate`'s job and stays there, so the
  // field is checked for being inside one rather than for a second sentence
  // of its own.
  const field = readSource('../src/components/features/browser-cloud/BrowserHomepageField.tsx')
  assert.match(field, /<ScopedSettingGate setting=\{setting\}>/)
  assert.match(
    renderToStaticMarkup(
      createElement(ScopedSettingGate, { children: null, setting: lockedAbove }),
    ),
