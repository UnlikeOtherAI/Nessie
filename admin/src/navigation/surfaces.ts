...
// where to return.
//
// Rulebook: `docs/navigation.md`. Plan: step 3 of
...
// The closed set of page types (plan §3). Overlays are not routes, so they
// are not here — they are painted over whatever screen a row below owns.
// `redirect` is not a page type: it marks a route that only forwards to
// another one, so it never classifies a screen (see `surfaceScreen`), but it
// still declares which section owns the moment it is on screen.
