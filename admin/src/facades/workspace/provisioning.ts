...
/**
 * Creating an organisation or a workspace without leaving Nessie.
 *
 * Both mutations do the same three things in the same order — ask UOA to
 * create, switch onto what UOA answered, land in the new workspace — because
...
const useProvisionAndSwitch = (path: string) => {
  ...
  return useMutation({
    mutationFn: async (input: CreateInput) => {
      const created = await apiClient.post<ProvisionedWorkspace>(path, input)
...
export const useCreateOrganization = () =>
  useProvisionAndSwitch('/api/workspaces/organizations')

/** Add a workspace to the organisation the person is currently in. */
export const useCreateWorkspaceTeam = () =>
  useProvisionAndSwitch('/api/workspaces/teams')
