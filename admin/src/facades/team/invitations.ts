...
export const useAcceptTeamInvitation = () => {
  ...
  return useMutation({
    mutationFn: async (invite: UoaPendingTeamInvite) => {
      const accepted = await apiClient.post<AcceptTeamInvitationResponse>(
        `/api/team/invitations/${encodeURIComponent(invite.inviteId)}/accept`,
        {
