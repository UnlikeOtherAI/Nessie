...
export const ExecutorDesktopCompanionPanel = ({
  ...
  useEffect(() => {
    ...
    void executorCompanionStatus()
      .then((statuses) => {
        if (!current) return
        setAvailable(true)
        if (activeExecutorId) setStatus(statuses.find((entry) => entry.executorId === activeExecutorId) ?? null)
      })
      .catch(() => {
  ...
  if (!available || !activeExecutorId) return null
    ...
    <section className="admin-card grid gap-3 border border-[color:var(--accent)] p-4">
      ...
      {created ? (
        <button
          ...
          type="button"
        >
          {busy ? 'Pairing…' : 'Choose workspace and pair this Mac'}
        </button>
      ) : null}
