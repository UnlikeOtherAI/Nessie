...
export const ProductOperationsSection = ({
  ...
  const exposesOperationalCost = product.slug !== 'deep-water'
  ...
  return (
    <section className="border-t border-[var(--sep)] pt-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-[var(--tx2)]">
            Product activity recorded through Nessie&apos;s operational ledger for the current month.
          </p>
        </div>
      ...
      <div className={[
        'mt-3 grid gap-2',
        exposesOperationalCost ? 'sm:grid-cols-5' : 'sm:grid-cols-4',
      ].join(' ')}>
      ...
      {!exposesOperationalCost ? (
        <p className="mt-1 text-xs text-[var(--tx3)]">
          Deep Water commercial totals, tariffs, and charges come only from the
          UOA customer statement in Usage &amp; Billing.
        </p>
      ) : null}
    </section>
  )
