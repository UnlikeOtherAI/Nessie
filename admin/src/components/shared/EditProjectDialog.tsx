...
export const EditProjectDialog = ({ onClose, open, project }: EditProjectDialogProps) => {
  ...
  // Focus is the Dialog's job now (`initialFocusRef`), which narrows one case:
  // this effect used to end with `nameInputRef.current?.focus()`, so swapping the
  // dialog to a different project WHILE MOUNTED re-focused the Name field.
  // `useModalA11y` focuses on open only. Both call sites gate on a target being
  // set and close between projects, so the swap-in-place path is unreachable
  // today — recorded rather than reinstated, because reinstating it would mean
  // focusing from outside the shell that owns focus.
  // Reseeding the fields when
  ...
  return (
    // The shell's default `md` geometry IS this dialog's shipped geometry: its
    // panel carried `w-full max-w-xl`, but `.create-channel-panel` (max-width
    // 440px, width calc(100% - 32px)) is unlayered CSS in styles.css while the
    // Tailwind utilities live in `@layer utilities`, and unlayered declarations
    // win over layered ones. Both utilities were inert; the panel rendered at
    // 440px, which is exactly `size="md"`.
    <Dialog
