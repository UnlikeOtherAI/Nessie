...
export const exerciseRichBoardAssignees = async ({
  ...
  try {
    ...
    await listbox.waitFor({ state: 'hidden' })
    await waitForTriggerFocus(page)
    const selectedRemoteLabel = trigger.locator('span[title]').filter({ hasText: REMOTE_NAME })
    await selectedRemoteLabel.waitFor()
    assert.equal(await selectedRemoteLabel.innerText(), REMOTE_NAME, 'selected remote person stays named in the trigger')
    assert.equal(await selectedRemoteLabel.getAttribute('title'), REMOTE_NAME, 'selected remote person exposes their full name')
