...
const main = async () => {
  ...
  try {
    ...
    const taskNoteTitle = `Board B note ${runId}`
    await secondDialog.getByRole('button', { name: 'New note', exact: true }).click()
    await secondDialog.getByRole('textbox', { name: 'Note title' }).fill(taskNoteTitle)
    await secondDialog.getByRole('button', { name: 'Create', exact: true }).click()
    await desktopPage.page.waitForURL(new RegExp(`/projects/${project.id}/docs`, 'u'))
    await desktopPage.page.getByLabel('Page breadcrumbs').getByText(taskNoteTitle, { exact: true }).waitFor()
    await shot(desktopPage.page, 'desktop-task-document-project-docs')
    await desktopPage.page.goBack()
    await waitForTaskLocation(desktopPage.page, project.id, boardB.id, boardBTask.id, true)
    await waitForTaskDetail(desktopPage.page, boardBTask)
