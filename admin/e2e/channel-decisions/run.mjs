...
try {
  ...
  await open()
  await page.getByRole('tab', { name: 'Channel', exact: true }).click()
  await page.getByRole('checkbox', { name: /Read-only for members/ }).check()
  await page.getByRole('checkbox', { name: /Mandatory admin announcements/ }).check()
  await page.evaluate(() => window.__channelDecisionsFixture.failNextSave())
  await page.getByRole('button', { name: 'Save', exact: true }).click()
  await page.getByRole('alert').filter({ hasText: 'Unable to save channel. Try again.' }).waitFor()
  await page.getByRole('button', { name: 'Save', exact: true }).click()
  await page.getByRole('dialog').waitFor({ state: 'hidden' })
  const announcementSave = await page.evaluate(() => window.__channelDecisionsFixture.writes.at(-1))
  assert.equal(announcementSave.adminOnlyPosting, true)
  assert.equal(announcementSave.mandatoryAnnouncements, true)
  assert.equal('topic' in announcementSave, false, 'announcement settings must not replace metadata')

  await open()
  await page.getByRole('tab', { name: 'Channel', exact: true }).click()
  await page.getByRole('checkbox', { name: /Read-only for members/ }).scrollIntoViewIfNeeded()
  await page.screenshot({ path: resolve(screenshots, 'desktop-announcement-settings.png') })
  await page.setViewportSize({ width: 390, height: 844 })
  await page.getByRole('checkbox', { name: /Mandatory admin announcements/ }).scrollIntoViewIfNeeded()
  await page.screenshot({ path: resolve(screenshots, 'phone-announcement-settings.png') })
  await page.getByRole('tab', { name: 'Agent decisions', exact: true }).click()
  ...
  await page.getByRole('dialog').waitFor({ state: 'hidden' })
  await page.evaluate(() => window.__channelDecisionsFixture.refresh({
    viewerCanManage: false, viewerCanConfigureAnnouncements: true,
  }))
  await page.getByRole('button', { name: 'Channel settings', exact: true }).click()
  assert.equal(await page.getByLabel('Name', { exact: true }).isDisabled(), true)
  assert.equal(await page.getByRole('tab', { name: 'Agent decisions' }).count(), 0)
  await page.getByRole('checkbox', { name: /Read-only for members/ }).uncheck()
  await page.getByRole('button', { name: 'Save', exact: true }).click()
  await page.getByRole('dialog').waitFor({ state: 'hidden' })
  const teamAdminSave = await page.evaluate(() => window.__channelDecisionsFixture.writes.at(-1))
  assert.equal(teamAdminSave.adminOnlyPosting, false)
  assert.equal('label' in teamAdminSave, false)
  await page.evaluate(() => window.__channelDecisionsFixture.refresh({
    viewerCanManage: false, viewerCanConfigureAnnouncements: false,
  }))
  await page.getByRole('button', { name: 'Channel settings', exact: true }).click()
