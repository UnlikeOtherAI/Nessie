...
try {
  ...
  await page.getByLabel('Model', { exact: true }).selectOption('local')
  assert.equal(await page.locator('option[value="setup"]').isDisabled(), true)
  await page.getByRole('link', { name: 'Set up a local model' }).waitFor()
  ...
  await page.getByRole('button', { name: 'Create batch job', exact: true }).click()
  ...
  await headerAction('Pause batch job')
  await page.getByRole('button', { name: 'Resume batch job', exact: true }).waitFor()
  ...
  await event('denied')
  await headerAction('Resume batch job')
  ...
  await page.getByLabel('Model', { exact: true }).selectOption('hosted')
  await page.getByLabel('Parallel requests across batch jobs', { exact: true }).fill('3')
  ...
  await page.getByLabel('Agent', { exact: true }).selectOption('00000000-0000-4000-8000-000000000007')
  await page.getByLabel('Conversation', { exact: true }).selectOption('00000000-0000-4000-8000-000000000008')
  await page.getByLabel('Instructions', { exact: true }).fill('Write a report from the completed spreadsheet.')
  ...
  dialog = page.getByRole('dialog', { name: 'Batch job item' })
  ...
  await page.getByRole('dialog', { name: 'Batch job item' }).getByRole('button', { name: 'Close', exact: true }).click()
  await page.getByRole('dialog', { name: 'Batch job item' }).waitFor({ state: 'hidden' })
  await event('waiting')
  ...
  assert.equal(await page.getByText('Edit batch job', { exact: true }).count(), 0)
  ...
  assert.equal(await page.getByRole('dialog', { name: 'Batch job item' })
