import assert from 'node:assert/strict'
import { mkdir } from 'node:fs/promises'
import { resolve } from 'node:path'
import { launchBrowser } from '../navigation/lib/browser.mjs'
import { REPO_ROOT } from '../navigation/lib/config.mjs'

const output = resolve(REPO_ROOT, 'e2e/screenshots/member-management')
const browser = await launchBrowser()
await mkdir(output, { recursive: true })
const calls = (page) => page.evaluate(() => window.memberManagementCalls)
const tab = async (page, label) => {
  const direct = page.getByRole('tab', { name: label, exact: true })
  if (await direct.isVisible()) await direct.click()
  else {
    await page.getByRole('button', { name: 'Member status', exact: true }).click()
    await page.getByRole('option', { name: label, exact: true }).click()
  }
}
try {
  for (const width of [1280, 390]) {
    const context = await browser.newContext({ viewport: { width, height: 844 }, hasTouch: width === 390 })
    // The fixture has no signed-in account; a 401 is the real anonymous session result.
    await context.route('**/api/auth/me', (route) => route.fulfill({ status: 401 }))
    const page = await context.newPage()
    const errors = []
    page.on('pageerror', (error) => errors.push(error.message))
    const open = async (params = '') => {
      await page.goto(`http://localhost:5455/e2e/member-management/index.html${params}`)
      await page.getByRole('heading', { name: 'People', exact: true }).waitFor()
    }
    const member = () => page.getByRole('button', { name: 'Open Jakub Rafaj', exact: true })
    const dialogClosed = () => page.getByRole('dialog').waitFor({ state: 'hidden' })
    // Screenshots wait for the finite motions in flight (a dialog opening, a
    // toast arriving) so they show the settled screen rather than a fade.
    const settled = () => page.evaluate(() => Promise.all(document.getAnimations()
      .filter((animation) => animation.effect?.getTiming().iterations !== Infinity)
      .map((animation) => animation.finished.catch(() => undefined))))
    // A confirmation is a toast in the shell's one polite live region: the
    // dialog that made the change has closed, so the toast is the only word.
    const confirmation = async (title, body) => {
      const card = page.locator('.card-viewport [role="status"]')
        .filter({ hasText: title }).filter({ hasText: body }).first()
      await card.waitFor()
      assert.equal(await page.locator('.card-viewport').getAttribute('aria-live'), 'polite')
      await settled()
      return card
    }
    await open()
    await page.locator('button[aria-label="Alerts"]').click()
    await page.getByText('Automatic access to Design needs reauthorization', { exact: true }).click()
    await page.waitForFunction(() => (
      window.location.pathname === '/admin/people'
      && new URLSearchParams(window.location.search).get('tab') === 'automatic'
    ))
    const repair = page.getByRole('button', { name: 'Re-authorize Design', exact: true })
    await repair.waitFor()
    assert.equal(await page.locator('[data-alert-target="true"]').count(), 1)
    assert.equal(await page.evaluate(() => document.activeElement?.getAttribute('data-alert-target')), 'true')
    await page.screenshot({ path: resolve(output, `${width}-automatic-membership-alert-repair.png`) })
    await repair.click()
    assert.ok((await calls(page)).some((call) => call.path.endsWith('/rules/10000000-0000-4000-8000-000000000015/reauthorize')))
    // The repair stays on its owning Automatic access tab; resume the
    // pre-existing roster assertions from their Active users home.
    await open()
    await member().click()
    await page.getByLabel('Role', { exact: true }).selectOption('admin')
    await settled()
    await page.screenshot({ path: resolve(output, `${width}-role-change.png`) })
    await page.getByRole('button', { name: 'Save changes', exact: true }).click()
    await dialogClosed()
    assert.ok((await calls(page)).some((call) => call.path.endsWith('/subject-jakub/role') && call.body.role === 'admin'))
    await member().click()
    assert.equal(await page.getByLabel('Role', { exact: true }).inputValue(), 'admin')
    await page.getByRole('button', { name: 'Deactivate in organisation', exact: true }).click()
    await page.getByText('They won’t be able to use any team in your organisation until you reactivate them.').waitFor()
    await page.getByTestId('confirm-dialog-confirm').click()
    await dialogClosed()
    await tab(page, 'Deactivated members')
    await member().click()
    await page.getByRole('button', { name: 'Reactivate in organisation', exact: true }).click()
    await page.getByTestId('confirm-dialog-confirm').click()
    await dialogClosed()
    await tab(page, 'Active members')
    await member().click()
    await page.getByRole('button', { name: 'Remove from team', exact: true }).click()
    await page.getByTestId('confirm-dialog-cancel').click()
    assert.equal((await calls(page)).some((call) => call.method === 'DELETE'), false)
    await page.getByRole('button', { name: 'Remove from team', exact: true }).click()
    await page.getByTestId('confirm-dialog-confirm').click()
    await dialogClosed()
    await page.getByText('No active members', { exact: true }).waitFor()
    // Adding someone already in the organisation closes the dialog and says so.
    await page.getByRole('button', { name: 'Invite people', exact: true }).click()
    await page.getByLabel('Search your organisation', { exact: true }).fill('Ondřej')
    await page.getByRole('button', { name: /Ondřej Novák/ }).waitFor()
    await settled()
    await page.screenshot({ path: resolve(output, `${width}-add-existing-member.png`) })
    await page.getByRole('button', { name: /Ondřej Novák/ }).click()
    await dialogClosed()
    assert.ok((await calls(page)).some((call) => call.method === 'POST'
      && call.path === '/api/team/members' && call.body?.uoaSub === 'subject-ondrej'))
    await confirmation('Member added', 'Ondřej Novák is now in your team.')
    await page.screenshot({ path: resolve(output, `${width}-member-added-confirmation.png`) })

    for (const scope of ['team', 'organization']) {
      await open(`?scope=${scope}`)
      await page.getByRole('button', { name: 'Invite people', exact: true }).click()
      assert.equal(await page.getByText('Set up automatic team access', { exact: true }).count(), 0)
      if (scope === 'team') await page.getByRole('tab', { name: 'Invite by email', exact: true }).click()
      else {
        // Organisation invitations pick any number of teams; the team form
        // invites into its own team and therefore has no team list at all.
        const design = page.getByRole('checkbox', { name: 'Design', exact: true })
        const research = page.getByRole('checkbox', { name: 'Research', exact: true })
        await page.getByRole('button', { name: 'Select all', exact: true }).click()
        assert.ok(await design.isChecked() && await research.isChecked())
        await page.getByRole('button', { name: 'Deselect all', exact: true }).click()
        assert.ok(!(await design.isChecked()) && !(await research.isChecked()))
      }
      assert.equal(await page.getByLabel('Name (optional)', { exact: true }).getAttribute('maxlength'), '120')
      await page.getByLabel('Email', { exact: true }).fill('new@example.test')
      if (scope === 'organization') {
        await page.getByRole('button', { name: 'Send invitation', exact: true }).last().click()
        await page.getByText('Choose at least one team.', { exact: true }).waitFor()
        assert.equal((await calls(page)).some((call) => call.body?.email !== undefined), false)
        await page.getByRole('checkbox', { name: 'Research', exact: true }).check()
        await page.getByRole('button', { name: 'Select all', exact: true }).click()
      }
      await settled()
      await page.screenshot({ path: resolve(output, `${width}-${scope}-invitation-name-limit.png`) })
      await page.getByRole('button', { name: 'Send invitation', exact: true }).last().click()
      await dialogClosed()
      const sent = (await calls(page)).find((call) => call.method === 'POST' && call.body?.email === 'new@example.test')
      assert.ok(sent)
      // Sending names the address and, at organisation scope, every team.
      await confirmation('Invitation sent', scope === 'organization'
        ? 'new@example.test is invited to Design and Research.'
        : 'new@example.test is invited to your team.')
      await page.screenshot({ path: resolve(output, `${width}-${scope}-invitation-sent.png`) })
      if (scope === 'organization') assert.deepEqual([...sent.body.teamIds].sort(), ['team-external', 'team-research'])
      else assert.equal(sent.body.teamIds, undefined)
      // Submit the same person to the same target again. The admin sends both
      // requests through to UOA, which owns the atomic resend/no-duplicate
      // behaviour; the fixture retains one actionable pending row.
      await page.getByRole('button', { name: 'Invite people', exact: true }).click()
      assert.equal(await page.getByText('Set up automatic team access', { exact: true }).count(), 0)
      if (scope === 'team') await page.getByRole('tab', { name: 'Invite by email', exact: true }).click()
      else await page.getByRole('checkbox', { name: 'Design', exact: true }).check()
      await page.getByLabel('Email', { exact: true }).fill('new@example.test')
      await page.getByRole('button', { name: 'Send invitation', exact: true }).last().click()
      await dialogClosed()
      const repeated = (await calls(page)).filter((call) =>
        call.method === 'POST' && call.body?.email === 'new@example.test')
      assert.equal(repeated.length, 2)
      if (scope === 'organization') assert.deepEqual(repeated[1]?.body.teamIds, ['team-external'])
      await tab(page, 'Pending invitations')
      await page.getByRole('button', { name: 'Open invitation for new@example.test' }).first().waitFor()
      // One actionable row per invited team: both for the organisation's
      // Select all, the fixture's one team otherwise.
      assert.equal(await page.getByRole('button', { name: 'Open invitation for new@example.test' }).count(),
        scope === 'organization' ? 2 : 1)
      await page.getByRole('button', { name: 'Open invitation for pending@example.test' }).click()
      await page.getByRole('heading', { name: 'Pending invitation', exact: true }).waitFor()
      await settled()
      await page.screenshot({ path: resolve(output, `${width}-${scope}-pending-invitation.png`) })
      await page.getByRole('button', { name: 'Resend', exact: true }).click()
      await dialogClosed()
      const resent = (await calls(page)).find((call) => call.path.endsWith('/invite-existing/resend'))
      assert.ok(resent)
      if (scope === 'organization') assert.equal(resent.body.teamId, 'team-external')
      await confirmation('Invitation sent again', 'We’ve emailed pending@example.test again.')
      await page.getByRole('button', { name: 'Open invitation for pending@example.test' }).click()
      await page.getByRole('button', { name: 'Cancel invitation', exact: true }).click()
      await page.getByRole('heading', { name: 'Cancel invitation?', exact: true }).waitFor()
      await page.getByText('Pending won’t be able to use it to join Design.', { exact: true }).waitFor()
      await page.getByText('You can invite them again later.', { exact: true }).waitFor()
      await settled()
      await page.screenshot({ path: resolve(output, `${width}-${scope}-cancel-invitation.png`) })
      await page.getByRole('button', { name: 'Cancel invitation', exact: true }).click()
      await dialogClosed()
      assert.ok((await calls(page)).some((call) => call.path.endsWith('/invite-existing/revoke')))
      await confirmation('Invitation cancelled', 'You can invite pending@example.test again at any time.')
    }
    await open('?scope=organization')
    await member().click()
    await page.getByLabel('Organisation role', { exact: true }).selectOption('admin')
    await settled()
    await page.screenshot({ path: resolve(output, `${width}-organization-role-change.png`) })
    await page.getByRole('button', { name: 'Save changes', exact: true }).click()
    await dialogClosed()
    assert.ok((await calls(page)).some((call) => call.path.endsWith('/organization/members/subject-jakub/role')
      && call.body.role === 'admin'))
    await member().click()
    assert.equal(await page.getByLabel('Organisation role', { exact: true }).inputValue(), 'admin')
    const designTeam = page.getByRole('checkbox', { name: 'Design', exact: true })
    for (let attempt = 0; attempt < 40 && !(await designTeam.isChecked()); attempt += 1) {
      await page.waitForTimeout(25)
    }
    assert.equal(await designTeam.isChecked(), true)
    await designTeam.uncheck()
    const teamAccessCalls = (await calls(page)).filter((call) =>
      call.method === 'GET' && new URL(call.path, 'http://localhost').pathname.endsWith('/teams')).length
    await page.evaluate(() => window.refetchMemberManagementQueries())
    assert.equal((await calls(page)).filter((call) =>
      call.method === 'GET' && new URL(call.path, 'http://localhost').pathname.endsWith('/teams')).length > teamAccessCalls, true)
    assert.equal(await designTeam.isChecked(), false)
    assert.equal(await page.getByRole('button', { name: 'Save changes', exact: true }).isDisabled(), false)
    await page.getByRole('button', { name: 'Save changes', exact: true }).click()
    await dialogClosed()
    await member().click()
    assert.equal(await page.getByRole('checkbox', { name: 'Design', exact: true }).isChecked(), false)

    await open('?readOnly=1')
    await member().click()
    assert.equal(await page.getByLabel('Role', { exact: true }).isDisabled(), true)
    assert.equal(await page.getByRole('button', { name: 'Remove from team', exact: true }).count(), 0)
    await page.getByRole('button', { name: 'Cancel', exact: true }).click()
    await dialogClosed()
    assert.equal(await page.getByRole('button', { name: 'Invite people', exact: true }).count(), 0)
    await tab(page, 'Pending invitations')
    await page.getByRole('button', { name: 'Open invitation for pending@example.test' }).click()
    assert.equal(await page.getByRole('button', { name: 'Cancel invitation', exact: true }).count(), 0)
    assert.equal(await page.getByRole('button', { name: 'Resend', exact: true }).count(), 0)
    assert.equal((await calls(page)).some((call) => call.method !== 'GET'), false)

    await open('?noPermissions=1')
    assert.equal(await page.getByRole('button', { name: 'Invite people', exact: true }).count(), 0)
    await member().click()
    assert.equal(await page.getByLabel('Role', { exact: true }).isDisabled(), true)
    assert.equal(await page.getByRole('button', { name: 'Remove from team', exact: true }).count(), 0)
    assert.equal(await page.getByRole('button', { name: 'Deactivate in organisation', exact: true }).count(), 0)
    await page.getByRole('button', { name: 'Cancel', exact: true }).click()
    await tab(page, 'Pending invitations')
    await page.getByRole('button', { name: 'Open invitation for pending@example.test' }).click()
    assert.equal(await page.getByRole('button', { name: 'Resend', exact: true }).count(), 0)
    assert.equal(await page.getByRole('button', { name: 'Cancel invitation', exact: true }).count(), 0)

    await open('?scope=organization&noPermissions=1')
    await member().click()
    assert.equal(await page.getByLabel('Organisation role', { exact: true }).isDisabled(), true)
    assert.equal(await page.getByRole('button', { name: 'Save changes', exact: true }).isDisabled(), true)

    await open('?pendingApproval=1')
    await tab(page, 'Pending invitations')
    await page.getByRole('button', { name: 'Open invitation for pending@example.test' }).click()
    await page.getByText('Waiting for an organisation admin to approve it.', { exact: true }).waitFor()
    assert.equal(await page.getByRole('button', { name: 'Resend', exact: true }).count(), 0)
    assert.equal(await page.getByRole('button', { name: 'Cancel invitation', exact: true }).count(), 0)
    assert.equal(await page.getByRole('button', { name: 'Approve', exact: true }).count(), 0)
    assert.equal(await page.getByRole('button', { name: 'Deny', exact: true }).count(), 0)
    assert.equal((await calls(page)).some((call) => call.method !== 'GET'), false)

    await open('?fail=role')
    await member().click()
    await page.getByLabel('Role', { exact: true }).selectOption('admin')
    await page.getByRole('button', { name: 'Save changes', exact: true }).click()
    await page.getByText('Your permission to change this role has changed. Reload members and try again.').waitFor()
    await page.screenshot({ path: resolve(output, `${width}-role-refused.png`) })
    assert.equal(await page.getByLabel('Role', { exact: true }).inputValue(), 'admin')

    await open('?slow=1')
    await member().click()
    await page.getByLabel('Role', { exact: true }).selectOption('admin')
    await page.getByRole('button', { name: 'Save changes', exact: true }).click()
    await page.getByRole('button', { name: 'Saving…', exact: true }).waitFor()
    assert.equal(await page.getByRole('button', { name: 'Saving…', exact: true }).isDisabled(), true)
    assert.equal((await calls(page)).filter((call) => call.path.endsWith('/subject-jakub/role')).length, 1)
    await dialogClosed()

    await open('?fail=invite')
    await page.getByRole('button', { name: 'Invite people', exact: true }).click()
    await page.getByRole('tab', { name: 'Invite by email', exact: true }).click()
    await page.getByLabel('Email', { exact: true }).fill('draft@example.test')
    await page.getByRole('button', { name: 'Send invitation', exact: true }).last().click()
    await page.getByText('Your permission to send invitations has changed. Reload members and try again.').waitFor()
    assert.equal(await page.getByLabel('Email', { exact: true }).inputValue(), 'draft@example.test')
    // A refused send keeps the dialog and its draft, and confirms nothing.
    assert.equal(await page.locator('.card-viewport').count(), 0)

    await open('?scope=organization&fail=access')
    await member().click()
    await page.getByText('Team access could not be loaded.', { exact: false }).waitFor()
    assert.equal(await page.getByText('You don’t manage any teams.', { exact: true }).count(), 0)
    assert.equal(await page.getByRole('button', { name: 'Save changes', exact: true }).isDisabled(), true)
    await open('?fail=search')
    await page.getByRole('button', { name: 'Invite people', exact: true }).click()
    await page.getByLabel('Search your organisation').fill('Ondřej')
    await page.getByText('People could not be searched.', { exact: false }).waitFor()
    assert.equal(await page.getByText('No one to add by that name. Try inviting them by email.', { exact: true }).count(), 0)
    await page.getByRole('button', { name: 'Retry', exact: true }).click()
    assert.deepEqual(errors, [])
    console.log(`Member management ${width}px: role, activation, removal, invitations, confirmations, team access, permissions and failures passed`)
    await context.close()
  }
} finally {
  await browser.close()
}
